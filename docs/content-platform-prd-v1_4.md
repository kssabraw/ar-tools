...
```
proposed ──Approve──→ approved ──promotion──→ in_progress ──run.complete──→ published
   │                     │                           │
   │                     │                           └─run.failed──→ approved (last_promotion_failed_at set)
   │                     │
   ├──Reject──→ rejected
   │
...
| Status | Meaning |
|---|---|
| `proposed` | Initial state on persist. Surfaces in the default dashboard view. |
| `approved` | User clicked Approve; ready for promotion. Stays visible in default view. |
...
| `status` | `text` | enum: `proposed`, `approved`, `rejected`, `in_progress`, `published`, `superseded`; default `proposed` |
