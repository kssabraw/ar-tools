...
> - Added fanout-sourced auto-approval policy for silo candidates: when a persisted silo's `source_headings` contains at least one heading with `source` matching `llm_fanout_*`, the row is inserted with `status='approved'` instead of the historical `'proposed'` default. Implemented at the DB layer via a `BEFORE INSERT` trigger (`trg_silo_fanout_auto_approve`) so all writers inherit the policy uniformly. Auto-approval only flips status — it does NOT auto-dispatch a run; the user still chooses when to promote. Dedup hits preserve existing status. See updated §2 (Goals + Out of Scope), §7.7.2 (Status lifecycle), §8.5 (Per-candidate insert logic), §13 (Conventions table), and §14.1 (`silo_candidates.status` column note). New migration `20260507120000_fanout_sourced_silo_auto_approve.sql` ships the trigger.
>
...
**Fanout-sourced auto-approval.** A silo candidate is "fanout-sourced" when at least one entry in its `source_headings` has `source` matching `llm_fanout_*` (i.e., `llm_fanout_chatgpt`, `llm_fanout_claude`, `llm_fanout_gemini`, or `llm_fanout_perplexity` — see Brief Generator PRD v2.0 §6). On insert, fanout-sourced rows persist with `status='approved'` directly; non-fanout-sourced rows persist with `status='proposed'` (the historical default). This rule is enforced at the database layer via a `BEFORE INSERT` trigger on `silo_candidates` (see migration `20260507120000_fanout_sourced_silo_auto_approve.sql`) so the worker code does not need to branch on source type, and so any future writer of this table inherits the policy automatically. Auto-approval **only** sets the status — it does NOT dispatch a run; the user still chooses when to promote. Dedup hits do not change status (existing rows keep their current status regardless of the new brief's source mix).
...
| `suggested_keyword_embedding` | `vector(3072)` | text-embedding-3-large dimensions; unit-normalized |
| `status` | `text` | enum: `proposed`, `approved`, `rejected`, `in_progress`, `published`, `superseded`; column default `proposed`. **A `BEFORE INSERT` trigger (`trg_silo_fanout_auto_approve`) rewrites this to `approved` when any element of `source_headings` has `source` matching `llm_fanout_*`** — see Section 7.7.2 "Fanout-sourced auto-approval" and migration `20260507120000_fanout_sourced_silo_auto_approve.sql`. Trigger only fires when the inbound `status` is `proposed`, so explicit overrides are respected. |
...
| **1.4.1** | **2026-05-07** | **Fanout-sourced silo auto-approval. Silos whose `source_headings` include any heading with `source` matching `llm_fanout_*` (i.e., `llm_fanout_chatgpt`, `llm_fanout_claude`, `llm_fanout_gemini`, `llm_fanout_perplexity` per Brief Generator PRD v2.0 §6) now persist with `status='approved'` instead of `'proposed'`. Implemented at the DB layer via `BEFORE INSERT` trigger `trg_silo_fanout_auto_approve` so all writers inherit the policy uniformly and the dedup worker code does not branch on source type. Auto-approval only sets the status — it does NOT auto-dispatch a run; the user still chooses when to promote. Dedup hits preserve existing status on existing rows. Updated §2 (Goals + Out of Scope), §7.7.2 (Status lifecycle — added "Fanout-sourced auto-approval" subsection and revised the lifecycle diagram + status meanings), §8.5 (Per-candidate insert + dedup logic), §13 (Conventions table — new "Silo fanout-sourced auto-approval" row), and §14.1 (`silo_candidates.status` column note). Migration `20260507120000_fanout_sourced_silo_auto_approve.sql` ships the trigger; no schema-shape changes (the column default of `'proposed'` is unchanged — the trigger overrides on insert). No frontend changes required: the dashboard already renders `approved` as a first-class state. Rationale: LLM fan-out queries already represent curated cross-LLM consensus signal of intent, so they don't need a second human-review gate before becoming actionable.** |
