...
- **GBP Sniper** — **high power, low RD, low contextual relevance** links pointed directly at the GBP — the maps equivalent of a power blast at a site's homepage.
  - **The target is ALWAYS the Google Business Profile URL — never a page on the site.** A Sniper aimed at a landing page, service page or the homepage is a misconfigured run: it spends the budget, it does not feed the profile the proximity/authority signal the tactic exists to create, and it points high-power low-relevance links at a page that wants the opposite. If the intent is to strengthen a *page*, that is ordinary link building (see the Link Building SOP's Maps targets), not a Sniper.
