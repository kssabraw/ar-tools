...
- **Phase 3a — Tier A, same `locations.patch` endpoint — ✅ BUILT 2026-09-09 (shipped dark).** `website`, `labels`, `special_hours`, `moreHours`, `serviceArea`, `openInfo`. Each is a pure `build_*_patch` builder + validator + parser + a field card in `GbpProfile.tsx`, riding the *existing* `gbp_profile_edits` row, apply job (re-read-and-diff), reconciler, freeze gate, and history unchanged — the widest reach for the least new machinery (migration `20260909120000` only relaxes the `field` CHECK). `moreHours` types come from the same `categories.batchGet(view=FULL)` as the services picker (`parse_more_hours_types` + `GET …/more-hours-types`); `serviceArea` places are resolved to Google placeIds via `maps_geocode` (`POST …/resolve-places` — never a guessed id). `specialHours` ships as its own `special_hours` field (masks only `specialHours`), independent of the `hours` field. Extra-confirm gate on `serviceArea` + `openInfo=CLOSED_*`. **`serviceArea`'s v1 write shape is the one to re-verify live at activation** (a wrong shape → a `rejected` edit, never a silent bad write).
- **Phase 3b — Tier B, category-gated reads. `categories` ✅ BUILT 2026-09-09 (shipped dark); `attributes` ✅ BUILT 2026-09-09 (shipped dark, its own PR).** `categories` rides `locations.patch` like a Tier-A field (`updateMask=categories`) — the field's value rides under `categories_value` (the flat `categories` key stays the services picker), pure `build_categories_patch`/`parse_categories_value`/`parse_category_search` + a live catalog SEARCH (`categories.list` → `GET …/categories/search?q=`) so the operator resolves real `gcid`s, extra-confirm on a PRIMARY-category change, migration `20260909130000` (applied live). **It consumes the existing `category_gaps` finding for free** — categories is now a valid `field`, so the strategist loop's generic `stage_strategist_draft` stages a categories draft with no new wiring (see loop reach below). **`attributes`** is the one field that does NOT ride `locations.patch`: it uses the **separate** `getAttributes`/`updateAttributes` endpoint pair (a per-attribute `updateMask` naming `attributes/{id}` resource names, an `Attributes` resource keyed at `locations/{id}/attributes`) + the category-scoped `attributes.list` (availability picker, `GET …/attributes/available`), a distinct read path (best-effort in `read_current` — an attributes read failure sets `attributes_error`, never breaks the profile page), and value-typed values (BOOL/ENUM/URL/REPEATED_ENUM). So the service layer **branches** the read/write for attributes (`_run_apply_attributes`/`_run_sync_attributes`, `_is_attributes`), keeping the generic single-`updateMask` path byte-identical for every other field; an edit targets the **subset** of attributes it changes (the mask names just those, others untouched; a cleared attribute stays in the mask with an empty value). Pure `build_attributes_patch`/`parse_attributes`/`parse_attribute_metadata`/`attributes_diff` (whole-set drift, Q3)/`attributes_subset_applied` (per-proposed outcome check), migration `20260909140000` (applied live — only relaxes the `field` CHECK; attributes REUSE the `gbp_profile_apply`/`gbp_profile_sync` job types, no new async_jobs type). **The v1 attributes write shape is the one to re-verify live at activation** (a wrong `updateMask` shape → a `rejected` edit, never a silent bad write). The `AttributesCard` (`GbpProfile.tsx`) loads availability lazily + renders a per-value-type control (BOOL tri-state / ENUM select / REPEATED_ENUM checkboxes / URL input) and only saves the changed subset.
...
- ~~**Categories + attributes editing** (the other half of what `gbp_audit` flags).~~ ✅ BUILT (Phase 3b, 2026-09-09) — `categories` (#1044) + `attributes` (this PR).
