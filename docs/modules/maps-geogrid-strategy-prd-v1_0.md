...
**A1. Share of Local Voice (SoLV).** *Data already exists* — `maps_scan_results.competitors_above.grid` carries per-pin competitor ranks. Compute, per scan: client coverage % (pins ranked ≤3 / total) and each competitor's coverage %. Store a per-keyword + overall time-series; render a trend line. **Action** (`maps_solv_drop`): "Losing local share to {competitor} (you {x}% → they {y}%)." No new API calls. *~1.5 days.*
