...
**B1. Competitor GBP intelligence. — BUILT (on-demand; monthly auto-refresh deferred).** `gbp_service.get_business_details()` already fetches arbitrary businesses. `services/competitor_gbp.py` selects the top local-pack competitors from the latest scan's stored `competitors` leaderboards (`select_competitors`, capped by `competitor_gbp_max`=8), fetches each GBP, and inserts a time-series capture into `competitor_gbp_profiles` (migration `20260629160000`) via the async `competitor_gbp` job (best-effort per competitor). Endpoints `GET /clients/{id}/maps/competitor-intel` + `POST .../competitor-intel/refresh`; rendered as a "Competitor intelligence" panel in the Maps History tab (category/rating/reviews/top-3-pins/hours, refresh button). Foundation for B2. Each capture also classifies the **business type** — SAB (service-area) / physical / hybrid — via `gbp_service.classify_business_type`. The primary signal is **`address_hidden`** (Outscraper's **`area_service`** flag, captured in the GBP mapping via `_address_hidden`): a hidden storefront address ⇒ pure service-area business — so a SAB that still exposes a *registered* address is now correctly classified `sab` (not `hybrid`). It falls back to a heuristic (published service-area places ⇒ serves areas; a street address ⇒ storefront; both ⇒ hybrid) only when the flag is absent (e.g. GBP captured before this field was stored — refreshes pick up the tight signal). Stored on `competitor_gbp_profiles.business_type`, shown as a "Type" column (competitor-intel panel + the B6 scorecard, which also classifies the client). *Follow-up: monthly auto-refresh via the scheduler.*
...
**B6. Local Relevance Scorecard. — BUILT (added on request; deterministic).** For a tracked keyword (the service) + the client's location, scores — for the client AND each top competitor — whether each ranking signal actually aligns with what's tracked: do their **reviews mention the service / location** (B3 `reviews` text scan), does their **GBP link to a page about the service / location** (scrape the GBP website → term match), is their **GBP category the service or closely related** (token + synonym map), the **site DR**, and the **GBP-linked page's UR** (DataForSEO Backlinks summary). All matching is **deterministic** (`services/local_relevance.py` — `service_terms`/`location_terms`/`mentions`/`category_match`/`page_relevance`/`detect_relevance_gaps`, a small trade-synonym map; no LLM). Service = the Maps keyword; location = `rank_tracking_location` or a parsed GBP-address locality. Stored per (client, keyword, entity) in `local_relevance_scores` (migration `20260629210000`) via the async `local_relevance` job. Endpoints `GET /clients/{id}/maps/relevance` + `POST .../refresh?keyword=`; a "Local relevance scorecard" table in the Maps History tab (category match · reviews-mention-svc/loc · page-about-svc/loc · DR · page UR, you vs competitors). **Action** (`local_relevance`, top of the Maps tier): consolidates the client's own gaps where competitors are stronger (GBP category not the service, GBP links to a non-service page, reviews don't mention the service, DR/UR behind the competitor median). *Synonym map + address-locality parsing are deterministic and tunable.*
