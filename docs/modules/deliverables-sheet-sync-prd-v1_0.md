...
- **Known limitation — a hard crash mid-append can duplicate a row (at-least-once write).** The Sheets append (`google_sheets.append_row`) happens *before* the durable "written" marker on the sync-log row. If the worker process is hard-killed (a Railway redeploy or OOM) in the sub-second window after the append returns but before the log/job status writes commit, the job is left `running` and the log `pending`; the stale-job reaper re-queues the job (`job_worker._plan_reap`), and on re-run the still-`pending` log drives a second `append_row` → a **duplicate row** in the client sheet. The Sheets append has no idempotency key, so this is an accepted at-least-once gap — the notes-watch path avoids the same class of duplicate via an atomic `notifications.dedupe_key`, but a sheet append has no equivalent. **Impact is low**: a duplicate deliverable row a human deletes, never data loss, and only in the narrow crash window (a caught error instead marks the log `failed` and does not re-append unless the task is re-completed). Deliberately not guarded in v1 — the alternative (mark `written` before appending) trades a rare visible duplicate for a rare *silent lost row*, which is worse for the "never silently drop a deliverable" goal. If duplicates are observed in practice, the fix is a pre-append idempotency check (persist the intended row + an `appending` status before the append; on a re-queued run with that status, read back the sheet's recent rows and skip if the row already landed).
