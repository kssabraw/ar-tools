...
**Status (2026-09-14):** **Phase 0 MERGED** (PR #1069 — pure core + tables/RPC/per-tier job type, applied live). **Phase 1 (Tier 1: city × main-service) MERGED** (PR #1070). **Phase 2 (Tier 2: city × subservice) MERGED** (PR #1072 — all CI green). **Phase 3 (Tier 3: CDP × main-service) MERGED** (PR #1076 — all CI green: platform-api tests + lint & typecheck + Netlify; squash-merged to `main` as `fc1afa5`; the new census CDP integration, worker-only; migration `20260914130000_coverage_audit_cdp_cache.sql` applied live). **Phase 4 (Tier 4: CDP × subservice) MERGED** (PR #1078 — all CI green: ruff + mypy + pytest + Netlify; squash-merged to `main` as `e80ed24`; pure wiring — no new census integration, no migration). **The module is now FEATURE-COMPLETE — all four tiers ship.** **Live-verified on the deployed worker 2026-09-14** (WheelHouse IT Fort Lauderdale) — the CDP path works end-to-end and two real bugs surfaced-and-fixed during verification (PRs #1082 / #1084 / #1085). See **"Live verification (2026-09-14)"** below. **Adversarial-review hardening MERGED (PR #1089, `425d8d0`):** the blocking service-axis planner LLM now runs off the shared event loop via `asyncio.to_thread` (Tier 1/3 + the Tier 2/4 main-axis pass — matching the `_derive_subservice_axis` precedent); `enqueue_coverage_audit` dedups a fresh auto-run against an in-flight `pending`/`running` job for the same client+tier (a double-click/retry no longer stacks a second paid run); and the edit-service-axis route gained the `budget_remaining()` 429 preflight `start` already had. No behaviour change to the audit output. **Adversarial-review follow-up (this branch):** three findings from a fresh review of the Phase-2 merge were fixed — (F1) the shared `useResumableJob` hook now resyncs its `phase` when the storageKey changes, so switching tiers into a backgrounded audit reconnects the "Running…" indicator instead of showing a stale idle state (a no-op for the ~38 constant-key consumers; the `enqueue_coverage_audit` in-flight dedup from #1089 already neutralised the only *paid* consequence, a duplicate run); (F3) `_derive_subservice_axis` now records a service whose planner succeeded but yielded no composable subservice in a new `empty_services` provenance list + note (previously silently dropped from both `planned`/`failed`); (F2) a code comment now states honestly that the per-service subservice planner LLM calls are unmetered + not cached, so a reaper requeue re-runs them (cheap; only the DataForSEO demand step is the metered/idempotent one). **`census_cdp.py` adversarial-review round MERGED (PR #1092, squash-merged to `main` as `b2f5b3c`; ruff + mypy + pytest + Netlify green):** three findings in the CDP resolver fixed — (C1, HIGH) `resolve_cdp_axis` no longer caches an EMPTY per-state enumeration: a transient TIGERweb failure returned `[]` and `_write_state_cdps` wrote it fresh, so `state_cdps_cached` served zero CDPs to every client in that state for `coverage_cdp_cache_days` (365) — now guarded `if st_cdps:` (mirrors `census_demand.py`'s `if rows:`) with a degraded note on the empty case; (C2, MEDIUM) CDP containment verification switched from forward-geocoding `"<CDP>, <seed_state>"` to checking each CDP's **authoritative Census centroid** with `place_is_within_city` — the old query mislocated a CDP in a *non-seed* state of a multi-state footprint (Kansas City straddles MO/KS: a KS CDP queried as "…, Missouri" was dropped) and re-introduced Google name ambiguity; the centroid check is more correct, removes the per-CDP paid geocode, and `candidates` became a list so a same-named CDP across two footprint states resolves per-centroid (name-dedup at the end); (C3, LOW) `_fetch_state_cdps` pagination advances `resultOffset` by the server's returned row count, not the parsed (post-filter) count, so dropped rows don't re-request the tail. `prov` now carries `verified` (distinct containment-passers, pre-cap) + `axis_size` (post-cap). All census/geocode work stays keyless-census + one cached seed forward-geocode (the CDP meter is untouched); three regression tests added (`tests/test_census_cdp.py`). Remaining work is calibration only.
...
## Adversarial-review hardening — MERGED (PR #1089, `425d8d0` 2026-09-14)

A fresh adversarial review of the runner (Phase 1 + the Phase 2/3/4 code on top) found three actionable issues; all merged (no behaviour change to the audit output, full importable suite green):
- **Event-loop block (Medium).** `run_coverage_audit_tier` called `_derive_service_axis` — which runs the blocking planner LLM (`_plan_service_axis` → `llm.call_tool`) — directly on the shared asyncio loop the API + every job lane run on (worker = `asyncio.create_task` in the lifespan), stalling all concurrent HTTP + jobs for the call. Both call sites (Tier 1/3 and the Tier 2/4 main-axis pass) now run via `await asyncio.to_thread(_derive_service_axis, …)`, matching the `_derive_subservice_axis` precedent (which already threaded `_generate_service_pages`).
- **Duplicate runs (Low).** `enqueue_coverage_audit` had no in-flight dedup; a double-click/retry/second tab stacked a second **paid** audit. A fresh auto-run (`service_axis=None`) now reuses a `pending`/`running` `coverage_audit` job for the same `(client, tier)` with no edited axis; an edited re-run always starts fresh.
- **Edit budget preflight (Low).** The `PUT …/service-axis` route now mirrors `start`'s `budget_remaining() <= 0 → 429` preflight.
- **Not changed (deliberately):** the unchunked `fetch_cached_market` read was already fixed at the `keyword_market` layer in #1085; the `_fetch_demand` `location_code is None` branch is correct defensive input-guarding (unreachable from the one caller, kept).
- Tests: a Tier-1 run asserting the planner executes off the event-loop thread + four `enqueue` dedup cases (`tests/test_coverage_audit_service.py`). Merged coherently with the sitemap-override work (#1087) — `enqueue_coverage_audit` carries both the dedup and `sitemap_url`.
