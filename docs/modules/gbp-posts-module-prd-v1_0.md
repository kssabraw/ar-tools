...
- **Listing registration picker (built 2026-07-23).** Under OAuth nothing pre-populates `gbp_locations` (the service-account metrics `resolve_locations()` is gated on the dormant metrics path), so `services/gbp_locations_service.py` closes the gap: `resolve_connected_locations()` lists every listing the *connected* account manages (v1 Account Management `accounts.list` → v1 Business Information `accounts.locations.list`, `readMask=name,title,storefrontAddress,phoneNumbers,metadata`, paged) using whichever credential `gbp_auth` selects (OAuth or SA), flags each with the client it's already registered to, and is best-effort (returns `{locations, detail}` — never raises). `register_location()` upserts one resolved listing onto a client (`access_status='ok'`) so it appears in the Compose picker; `unregister_location()` drops it (posts cascade). API: `GET /gbp/available-locations` (staff, agency-wide), `POST /clients/{id}/gbp/register-location`, `DELETE /clients/{id}/gbp/locations/{row_id}` (`routers/gbp_posts.py`). Frontend: the `RegisterLocations` picker (replaces the old static "no location" notice) — "Find my listings" → searchable list with address/phone → "Use for this client"/"Reassign here", plus a "Manage listings" toggle on the tab bar to add/remove later. Pure `parse_location` unit-tested.
