...
- **Auth:** OAuth2 with scope `https://www.googleapis.com/auth/business.manage` (there is no narrower posts scope). **Two auth models are supported (built 2026-07-23), selected by config in `services/gbp_auth.py`:**
  - **OAuth user token (preferred for GBP; chosen for this agency — Workspace account, 2026-07-23).** Google's Business Profile API is OAuth-first — a bare service account is often NOT accepted as a listing Manager the way it is for GSC (this is *why* competitors like Local Viking / GMB Briefcase use "Sign in with Google": they act as the user's own account, which already manages the listings). With a Google **Workspace** agency account we publish an **Internal** OAuth app (no Google verification/CASA, refresh token doesn't expire) and authorize **once** as the agency account that already manages the client listings — no per-client OAuth, and no "add the SA as Manager" step. Config: `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` (+ `GOOGLE_OAUTH_REDIRECT_URI` for the in-app flow). **Two ways to get the refresh token:** (a) **in-app one-click Connect (preferred, built 2026-07-23)** — an admin clicks *Connect Google Business Profile* on the GBP Posts page, approves Google's consent, and the callback stores the refresh token in **`gbp_oauth_credentials`** (agency-level single row; migration `20260723110000`). Flow in `services/gbp_oauth.py` (signed-state CSRF, code exchange, userinfo email) + `routers/gbp_oauth.py` (`/gbp/oauth/status|start|callback|disconnect`; callback is public, state-signed) + the frontend `ConnectionBar`. Needs a **Web application** OAuth client with the redirect URI registered. (b) **CLI fallback** — `scripts/get_gbp_refresh_token.py` (Desktop-app client → localhost loopback → prints the token to set as `GBP_OAUTH_REFRESH_TOKEN`). `gbp_auth` reads the token DB-first (env override wins if set). When configured, it mints the API token from the refresh token; **GSC + GBP-metrics are untouched** (they keep the service account). **White-label + client-granted access (2026-07-23):** connect with a **neutrally-branded Google account** (ideally on a neutral-domain Workspace so the manager email a client sees reveals nothing) — the app UI is staff-only and posts carry no tool attribution, so the connected account's identity is the only client-visible surface. When a client adds that account as a **Manager**, the resulting pending invitation is accepted in-app via **`services/gbp_invitations.py`** (v1 Account Management `accounts.invitations.list`/`.accept`) + `GET/POST /gbp/oauth/invitations|accept-invitations` + an "Accept access invitations" button on the ConnectionBar — no logging into the neutral Google account.
  - **Service account (fallback).** The agency **service account** used for GSC: add the SA's `client_email` as a **Manager** on the Business Profile; the app authenticates as the SA (`GOOGLE_SERVICE_ACCOUNT_KEY`, `business.manage` scope). Same onboarding as GSC ("add this email") — used only when the OAuth vars are unset. May not work for GBP (see above); the verify script determines it.
  - Only token-minting differs between the two — every v4 REST call, the whole service, jobs, and frontend are identical. `scripts/verify_gbp_api_access.py` auto-detects OAuth env (else the SA key) and proves the same account/location/localPosts chain either way.
...
- **Client→listing auto-match (built 2026-07-23; each client maps to its one GBP — owner ruling, no browse).** Under OAuth nothing pre-populates `gbp_locations` (the service-account metrics `resolve_locations()` is gated on the dormant metrics path), so `services/gbp_locations_service.py` closes the gap. Rather than making the operator browse the whole account inventory, the page **auto-matches the client to its own Business Profile** from what the suite already captured (`clients.gbp.business_name` + the lat/lng in `clients.gbp.google_maps_uri`): `resolve_connected_locations()` lists every listing the *connected* account manages (v1 Account Management `accounts.list` → v1 Business Information `accounts.locations.list`, `readMask=name,title,storefrontAddress,phoneNumbers,latlng,metadata`, paged) via whichever credential `gbp_auth` selects (OAuth or SA); `match_client_location(client_id)` pins the match to the **exact same Business Profile the client dashboard shows**: first by exact identifier — the **CID** shared by `clients.gbp.google_maps_uri` (hex `…!1s0x..:0x<hex>` form) and the API listing's `metadata.mapsUri` (`?cid=<decimal>` form), or a `metadata.placeId` when the client has one (`parse_cid`/`find_exact_match`, pure) — and only **falls back** to **name similarity + geo proximity** when no exact key is available (`name_similarity` Jaccard over suffix-stripped tokens; `parse_latlng_from_maps_uri` + haversine `_geo_score` — geo disambiguates two same-name listings across cities, e.g. WheelHouse IT Fort Lauderdale vs Orlando). Returns `{client_label, matched, candidates, detail}` (`matched` = the exact/confident pick, `candidates` = all ranked, the fallback). `register_location()` upserts the chosen listing onto the client (`access_status='ok'`); `unregister_location()` drops it (posts cascade). All best-effort — never raises. API: `GET /clients/{id}/gbp/match-location` (staff), `POST /clients/{id}/gbp/register-location`, `DELETE /clients/{id}/gbp/locations/{row_id}`, plus `GET /gbp/available-locations` (agency-wide fallback) (`routers/gbp_posts.py`). Frontend: the `RegisterLocations` panel (replaces the old static "no location" notice) shows **one matched card** — "This client's Business Profile → Use this profile" — with a "Not the right listing? Show all" fallback and a per-assignment Change/Remove, plus a "Manage listings" tab-bar toggle. Pure `parse_location`/`name_similarity`/`parse_latlng_from_maps_uri`/`score_match`/`rank_matches` unit-tested.
...
- **Distilled Voice & Audience Card (built 2026-08-11):** every draft path (single AI draft, the bulk create-from-URL jobs, and scheduled drafts) now grounds on the client's **distilled Voice Card** — the same enforceable card the page writers use (`voice_card_service.get_voice_card`, cached on `clients.voice_card` keyed by a guide-fingerprint; the first bulk job distills, the rest hit the cache). A GBP-tuned `render_voice_card_block` injects it as a late, high-priority prompt block (tone, grammatical person, must-use / never-use / discouraged terms, audience pains/motivations/objections, CTA wording — THESE WIN on expression), and a deterministic `voice_forbidden_hits` (word-boundary never-use match) triggers one corrective rewrite pass when a forbidden term slips through (keeping the version with fewer hits). Best-effort: a client with no guide/ICP gets `{}` → the prior compact-context behaviour. Distinct from the page writers' full 8-dimension scoring/publish-gate (posts are short/low-risk — guided + forbidden-term-enforced, not scored). Pure `render_voice_card_block`/`voice_forbidden_hits` unit-tested.
...
- **Bulk "create N posts from a page URL" (built 2026-08-11):** a Compose panel takes any page **URL** + a count **0–99** and drafts that many **distinct** posts from the page's *actual content*. Backend `gbp_posts_service.enqueue_generate_from_url` fetches the page once (`syndication_rewrite.extract_source_content` → ScrapeOwl + chrome-strip → Markdown, capped at `gbp_post_source_chars`), then enqueues N staggered `gbp_post_generate` jobs (reusing the existing draft handler + reaper-safe spacing `gbp_post_bulk_spacing_seconds`), each carrying the page content + a rotating **angle** (`variation_instruction`, pure/unit-tested — biggest-benefit / a specific tip / a customer question / … cycled by index) so the posts don't paraphrase each other; the CTA defaults at the source page. `draft_summary` grounds each post ONLY on the fetched content (no invented facts/prices/dates). Drafts only — never auto-publishes. `POST /clients/{id}/gbp/posts/generate-from-url` → `{count, job_ids}`; the frontend polls the shared jobs/status batch and drafts appear in Posts. Count/clamp (`clamp_bulk_count`) + angle rotation unit-tested. Config: `gbp_post_source_chars`, `gbp_post_bulk_spacing_seconds`, `gbp_post_max_bulk`.
