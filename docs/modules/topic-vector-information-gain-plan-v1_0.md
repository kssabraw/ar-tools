...
**Status:** Design locked. **P0 built (report-only, 2026-09-16)** — topic centering + per-subtopic coverage + the inverse gain gap on the ecommerce scorer (`/score-ecommerce-page`, `/reoptimize-ecommerce-page`), gated on `GEMINI_API_KEY`, as a separate async measure beside the deterministic `_compute_serp_signal_coverage` (which is byte-identical); nothing is fed to the composite or the reopt loop yet. P1 (site claim index + scored information gain) and P2 (emotional-arc rubric) are not built. This doc remains the design authority.
