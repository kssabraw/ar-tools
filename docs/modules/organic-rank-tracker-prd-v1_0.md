...
| **M4 — DataForSEO + detection + alerts** | **Built:** live "Today" rank (the fallback, see §2) + CPC/volume/competition (`keyword_market`) + estimated-monthly-value ROI. **Remaining:** weekly query×page → `canonical_url` resolution, deindex gap detection + URL Inspection confirmation, alerting via the suite notifications service, striking-distance discovery from `gsc_query_daily`. | In progress |
