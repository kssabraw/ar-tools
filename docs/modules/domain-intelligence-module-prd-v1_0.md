...
**Authored:** 2026-07-11 · **Status:** **ALL PHASES (0–5) built.** Foundations + Domain Overview/Ranked Keywords + Keyword Gap + Backlink Gap + Discover + Phase 4 signals (scheduler/notifications/Action Plan) + Phase 5 agent wiring (a `domain_intel` provider in both the strategist digest and the SerMaStr Slack/dashboard context, so competitor gaps become raw material for strategist proposals and assistant Q&A). The **daily budget** stays at 200/day (owner-approved placeholder, 2026-07-12); "authority overtaken" is a deferred follow-up. A post-completion **adversarial-review hardening pass** (2026-07-12) fixed: the scheduler's last-run signal now reads the `async_jobs` history instead of gap rows (zero-gap clients and PostgREST row-cap truncation both caused daily paid re-runs), eligibility requires a website (website-less LeadOff clients caused perpetual failed jobs), all-competitor-fetch-failure raises instead of wiping the standing gap set, gap replacement is insert-new-then-delete-old (stamped per run), the due-check is fully exception-guarded (an escape would starve every scheduler hook after it), `ranked_keywords` pins `item_types: ["organic"]`, `/discover` is POST (paid call — GETs get retried by proxies), link-gap reads order NULLS LAST, first-run gap notifications follow the baseline convention (no whole-landscape alert), old snapshots are pruned to 4 per (client, domain), and the frontend keeps failure banners visible (job ids are no longer cleared on terminal status). · Competitive-intelligence module (the "SEMrush clone")
