...
**Status:** Shared core + Phases 1, **1.1, 2, 3, and 4** all **BUILT** (Phase 1 2026-09-15; Phase 1.1 + 2 + 3 + 4 continued 2026-09-15, PR #1109). Ships **dark** behind `google_trends_enabled` (code default False). Corrected against the adversarial review (the `item_types`/sandbox-reachability/category-count fixes below are now decisions in the code, not open risks). **⚠️ The live DataForSEO response shape is still UNVERIFIED** — every phase parses defensively against the documented shape; run `scripts/verify_google_trends.py` from Railway PLATFORM (both the rising-queries AND the Phase-4 interest_over_time slice) before flipping the flag.
...
- [ ] **BEFORE FLAG ON:** run `scripts/verify_google_trends.py` from Railway PLATFORM → confirm the `google_trends_queries_list` shape AND (for Phase 4) the `google_trends_graph` / interest_over_time shape; reconcile parser/fixtures if either differs (§2, §8.2). **Still outstanding as of the Phase 1.1–4 build.**
...
- [ ] Phase 4 follow-up: persist the Trends seasonality where `trend_watch.build_demand_outlook` reads it, so it surfaces on the Forecast card automatically (needs a dedicated column — must NOT overwrite `keyword_market.monthly_searches`)
