...
- [ ] **Follow-up:** the category scan derives good on-topic anchor seeds but they can be too long-tail for Trends to carry rising related queries (returns 0 even with the single-keyword fix). Consider deriving broader head-term seeds for the category scan.
