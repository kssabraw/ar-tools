...
- [x] **Follow-up (built):** the category scan derives good on-topic anchor seeds but they can be too long-tail for Trends to carry rising related queries (returns 0 even with the single-keyword fix). `google_trends.head_term_seeds` (pure, unit-tested) now reduces each derived category seed to its first ≤3 significant tokens (reusing `keyword_research.tokenize`, which already drops interrogatives + stopwords), deduped + capped, before exploring — applied ONLY in `run_google_trends_category_scan` (the relevance anchors keep the full phrases; user-entered Phase-1 seeds stay verbatim). Best-effort: an unshortenable seed falls through unchanged.
...
- [x] Phase 4 follow-up (built): persist the Trends seasonality where `trend_watch.build_demand_outlook` reads it, so it surfaces on the Forecast card automatically. New table `google_trends_seasonality` (client × keyword × location; `month_index` jsonb + peak/low months + `source_run_id` provenance; migration `20260916130000`, **applied live**) — `run_local_seasonal_scan` upserts each per-keyword profile into it. `trend_watch.build_demand_outlook` now PREFERS a stored Trends profile per keyword over `keyword_market.monthly_searches` (Ads volume history is NOT overwritten and still supplies the volume weighting), merged via the pure `merge_seasonality_profiles` so `demand_outlook`'s output shape — and the Forecast card — are unchanged. The win: a keyword whose Ads history is too thin for a seasonality profile becomes usable when it has a Trends profile. Read is best-effort (missing table → prior behaviour).
