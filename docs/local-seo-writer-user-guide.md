...
- **Reference page structures are optional but worth setting.** On the client form, you can point
  ...
  a page.
- **A frozen client can't generate, plan, or reoptimize.** Same rule as everywhere else in the
  suite — check the freeze banner if something's blocked.
...
4. For a service-area business with no fixed address, a **city** field appears so the page still
   gets a real anchor location.
...
- **Reoptimize tab** — paste one or more live URLs (single or bulk mode). Each page is scored
  first; **only pages below 75/100 are actually rewritten** — a page that's already strong is
  skipped with a note, so you're not spending effort on pages that don't need it. (Adding your own
  notes on a page forces a rewrite even if it's above threshold.)
...
soon"** — not live yet for reoptimize.
...
right on its preview:

- A headline verdict — "Sounds like this client," "Mostly on voice," "Drifting from the guide,"
  or "This does not sound like the client" — plus a 0–100 score.
...
- Eight scored dimensions, worst first, each with a real quoted excerpt from the page showing
  exactly where it drifted — so you're not guessing what to fix.

**If a client has no brand guide, this panel doesn't appear at all** — that's deliberate, so a
missing panel never gets mistaken for "checked and passed."
...
It only rewrites pages scoring below 75/100 by default. Add a note in the reoptimize form to force
a rewrite regardless of score.
