    ...
    <div style={{ border: '1px solid #e2e8f0', borderRadius: 10, padding: 14, display: 'flex', gap: 14 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          {draft.platform_metadata?.produced_by === 'autonomy' && (
            <span title="Generated by the Social Manager (autonomy loop)"
              style={{ background: '#f5f3ff', color: '#6d28d9', fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 999 }}>
              AI Manager
            </span>
          )}
...
// A compact read of the Social Manager (autonomy loop) ledger — renders nothing
// until the loop has actually run (it ships dark), so a client that's never used
// autonomy sees a clean Drafts tab.
function AutonomyActivity({ clientId }: { clientId: string }) {
  const q = useQuery<AutonomyRun[]>({
    queryKey: ['social-autonomy-runs', clientId],
    queryFn: () => api.get<AutonomyRun[]>(`/clients/${clientId}/social/autonomy-runs`),
  })
  const runs = q.data ?? []
  ...
  return (
    <div style={{ ...card, marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
        <div>
          <h3 style={{ margin: 0, fontSize: 15 }}>Social Manager activity</h3>
          <p style={{ margin: '2px 0 0', fontSize: 12, color: '#94a3b8' }}>What the autonomy loop generated, auto-queued, or proposed on recent runs.</p>
        </div>
        <button onClick={() => void q.refetch()} style={{ ...btn('#fff', '#334155'), padding: '6px 10px' }}><RefreshCw size={13} /> Refresh</button>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {runs.map((r) => (
          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', fontSize: 12, color: '#475569', borderTop: '1px solid #f1f5f9', paddingTop: 6 }}>
            <span style={{ color: '#94a3b8', minWidth: 128 }}>{r.at ? new Date(r.at).toLocaleString() : '—'}</span>
            <span style={{ background: '#f1f5f9', color: '#475569', padding: '2px 7px', borderRadius: 999, fontWeight: 600 }}>{r.trigger ?? 'run'}</span>
...
function DraftsTab({ clientId, accounts, angleSetId }: {
  ...
  return (
    <>
    <AutonomyActivity clientId={clientId} />
    <div style={card}>
