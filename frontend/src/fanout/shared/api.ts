...
// Merged suite: the Fan-out backend is mounted at `/fanout` on the platform API,
// so we derive the base from the same env var the rest of the suite uses
// (VITE_PLATFORM_API_URL) rather than the app's old standalone VITE_API_BASE_URL.
const BASE_URL = `${(import.meta.env.VITE_PLATFORM_API_URL as string) ?? ""}/fanout`;
...
export async function exportSelected(
  ...
  const resp = await fetch(
    `${BASE_URL}/sessions/${sessionId}/export-selected`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
