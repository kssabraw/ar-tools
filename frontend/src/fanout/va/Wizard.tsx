    ...
    <AppShell>
      <div className="content" style={{ maxWidth: 720 }}>
        ...
        {step === "deepmine" && sessionId && (
          <DeepMineStep
            sessionId={sessionId}
            // Seed from the carried selection so returning here from the cost
            // step (Back) preserves the ticked silos instead of resetting.
            initialGated={gated}
        ...
        {step === "progress" && sessionId && (
          <ProgressStep sessionId={sessionId} onDone={() => navigate(`/fanout/session/${sessionId}`)} />
        )}
      </div>
    </AppShell>
  );
...
function DeepMineStep(p: {
  ...
  // this step from the cost step keeps the previously ticked silos.
