...
## ⏩ Update — 2026-09-15 · **Brand Guide Generator — Phase 1 (capture, no browser) — BUILT, draft PR (branch `claude/brand-guide-gen-phase-1-d40je3`)** (latest)

Phase 1 of the Brand Guide Generator (PRD `docs/modules/brand-guide-generator-prd-v1_0.md` §4.1–4.2 / §6 / §10). Builds on the merged Phase 0 (#1138: the pure extraction core + the D4 palette spike, **gate PASSED live** — `css_recovered_any=true`, 4/4 sites captured). The ADR (no headless browser) stands. **Still gated on `brand_guide_enabled` (default False in code; currently TRUE on PLATFORM — owner flag decision pending, see below). Nothing auto-enqueues, so nothing spends until a guide is explicitly requested.**
...
**Owner decision pending — the `brand_guide_enabled` PLATFORM flag.** It is TRUE on PLATFORM today (set for the Phase-0 spike). Phase 1's new capture path is dark until this PR merges + deploys; leaving the flag TRUE is what lets a post-deploy worker-verification job actually run, and nothing auto-enqueues so there's no runaway-spend risk. Recommendation: **leave it TRUE**. Flip to FALSE only if you'd rather the whole module be inert until Phase 4's UI lands.
