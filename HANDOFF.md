...
- **Symptom B — the bulk batch itself takes hours to drain.** This is throughput-bound, and the ONLY levers are `bulk_lane_workers` + Anthropic capacity. Recipe, in order:
  ...
  2. **Raise `bulk_lane_workers` ONE step** (env on PLATFORM; default 1). Confirm the deploy goes ACTIVE (Railway trap — don't assume). Re-watch the gauge under a real batch.
