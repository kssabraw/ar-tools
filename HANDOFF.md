...
**Finding #2 — goal-aware Action Plan ordering — BUILT** (branch `claude/reopt-goal-aware-boost-fi5vpy`): `reopt_planner.build_plan` sorted by a FIXED tier order for every client, so a client measured on `gbp_calls`/`maps_pack_presence` got a plan topped by organic drops + cannibalization with its GBP/Maps levers stuck at tier 3. The approved **capped cross-tier boost** exploits the 1-unit gap between the cannibalization band (`[4×,4×+9999]`) and the offpage band (`[5×,…]`): a NON-emergency action (`sort < _SORT_OFFPAGE`) whose channel matches a **behind/overdue** goal is remapped into the fractional window `(_SORT_OFFPAGE-1, _SORT_OFFPAGE)` = `(49999, 50000)` — strictly above every cannibalization/quick/hidden row, strictly below every emergency (sitewide/deindex/drops/offpage), original strength preserved as the in-window fraction. Pure helpers `action_channel` (kind→organic/maps/ai), `goal_channels` (behind/overdue goals' channels only, so a healthy client keeps the default order), `apply_goal_boost` (the remap; no matched channels ⇒ byte-identical); `build_plan` loads `assess_goals` best-effort and applies the boost before the final sort. Gated on `reopt_goal_boost_enabled` (default True). No emergency tier touched, no autonomy guardrail touched (autonomy still reads `kind`, not order). 72 reopt_planner + 52 autonomy/drop-classifier tests green locally.

**Next:** #3 — proposal-only autonomy goal-lever routing, off #2.

## ⏩ Update — 2026-09-01 · **Goal-driven audit follow-ups — #929 (behind-goal trigger) MERGED; #4 stuck-proposal Director seam BUILT (PR pending)**
