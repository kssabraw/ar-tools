...
## ⏩ Update — 2026-09-01 · **SerMaStr module-coverage audit + Director context fix (PR [#925](https://github.com/kssabraw/ar-tools/pull/925), draft — CI green, mergeable clean, NOT YET MERGED) — open follow-up: module cards only exist for 3 of ~30+ modules** (latest)

Prompted by an owner request ("SerMaStr should have access to all modules"), this session audited SerMaStr's module coverage **against the actual code**, not the CLAUDE.md prose — which turned out to be incomplete/stale on this point (an earlier answer in this session wrongly claimed backlink data wasn't covered; it already was).

**Coverage audit finding:** `services/slack_assistant/context.py` already registers **34 context providers** — organic rank, maps/geo-grid, AI visibility, forecasting, trends, competitive intelligence, domain intelligence, backlink research (`_ctx_backlinks`), campaign goals, budget/task plan, QA, citations, syndication, reports, GBP metrics, GA4, SOPs, Asana, websites, ecommerce, GBP posts, native tasks, LeadOff, response episodes, setup, director, memories, and more. Near-total coverage already existed.

**One real gap found + fixed (PR #925):** the Director-of-Operations read model (`services/director/read_model.py::build_read_model`) already computes `interventions` (worked/partial/no_effect verdicts on past link-building/reoptimization proposals) and `autonomy` (executed/proposed/escalated counts + unactioned proposals) — but `_ctx_director` discarded both and only passed through `seam_flags`. So SerMaStr could see "there's a stuck proposal" but couldn't answer "did our last reoptimization actually move the keyword," even though the data was one already-made call away.
- `_ctx_director` now also surfaces `intervention_outcomes` (`by_verdict`, `enrolled`, `open_count`) and `autonomy_loop` (`executed`, `proposed`, `escalated`, `unactioned_count`), returning `None` only when all three (seam flags, interventions, autonomy) are empty.
- System prompt (`services/slack_assistant/prompts.py`) updated so SerMaStr knows to cite the new fields.
- 3 new unit tests in `tests/test_slack_assistant.py`; full `test_slack_assistant.py` (166) + the Director suite (28) green, no regressions.
- **PR #925 is a draft, all 6 checks green (mypy/ruff/pytest/3× Netlify), `mergeable_state: clean`. Not merged — waiting on the owner to mark ready/merge.**

**Open follow-up, NOT started — decide in the next session:** a deeper "how to correctly read this module's data" layer exists at `docs/agents/module-cards/` (vendored to `writer/platform-api/agent_docs/module-cards/`) — direction conventions, null-handling rules, known blind spots, and a worked misreading example per module (e.g. the rank-tracker card warns against reading `gsc_position: null` as "we lost the ranking," and against splicing GSC's impression-weighted average with DataForSEO's point-sample reads into one trend). **Only 3 cards exist**: `rank-tracker.md`, `geogrid-tracker.md`, `labs-ai-visibility.md`. Forecasting, Domain Intelligence, Competitive Intelligence, Keyword Research, LeadOff, Ecommerce, Website Builder, Campaign Goals, and the rest have no card — they rely only on lighter inline `note`/docstring guidance baked into each `_ctx_*`/`_prov_*` provider function, plus general system-prompt instructions. This is a real (if second-order) limit on how reliably SerMaStr interprets modules outside the 3 covered.
- Suggested starting set if the owner wants this built out: **Forecasting, Domain Intelligence, Campaign Goals** (most load-bearing in a strategy review), then Competitive Intelligence and LeadOff.
- Format to mirror: see `docs/agents/module-cards/rank-tracker.md` for the target shape (What it measures / instrument(s) + how they disagree / direction / how to read the fields / known blind spots / one worked misreading).
- Registration: a new card just needs to be dropped in `docs/agents/module-cards/` **and mirrored byte-identically into `writer/platform-api/agent_docs/module-cards/`** — `sop_library.load_module_cards()` loads the whole directory, and a unit test enforces the two copies stay in sync (same pattern as the SOP corpus).

**How SerMaStr actually uses SOPs (confirmed this session, for context on the above):** two real paths, not just prompt-stuffing. Conversational (Slack/web): `wants_sop_grounding` gates per-question whether SOP text loads (defaults True), `sop_domains` picks which SOPs, and a `read_sop` tool is always available to pull more. Strategist Review (weekly/on-demand/escalation): the SOP corpus (domain-relevance-selected) + DB per-client overrides + all module cards assemble into the prompt with a "whole doc or defer to `read_sop`" rule (never a truncated fragment) before a bounded tool-use loop and a forced `emit_strategy_review` call whose schema requires an `sop_citation` per finding/proposal (empty string allowed, but must be explicit).
