...
## ⏩ Update — 2026-09-15 · **Brand Guide Generator — module PRD DRAFTED + design-grilled + adversarially reviewed + MERGED to `main` (PR [#1128](https://github.com/kssabraw/ar-tools/pull/1128), squash `342856f`); NOT built** (latest)

Seeded by an owner review of a Google **Pomelli** "Brand Book" export (Nova Life Peptides) that read thin — visual-only, no audience, no worked voice examples, no messaging layer, ungrounded. New module scoped to clear that bar decisively by fusing **measured live-site visual identity** + a **vision "vibe" read** + the suite's existing **voice/ICP/differentiator** assets + a synthesized **messaging** layer into one **Brand Audit + Brand Guide** — a client-facing PDF (soft sales asset) AND a structured record that can *suggest* refinements to the enforcement assets. Authoritative: **`docs/modules/brand-guide-generator-prd-v1_0.md`** + the ADR in **`decisions.md`** ("Brand Guide Generator — no headless browser for visual extraction").

- **Status: PROPOSED, not built.** Gated on `brand_guide_enabled` (default False). ~70% is assembly over existing `brand_voice`/`detected_icp`/`voice_card` + the `client_report` WeasyPrint PDF scaffold; the one genuinely new capability is **live-URL visual extraction** (nothing in the suite extracts colors/fonts/logo from a live URL today).
- **Owner decisions (PRD §2):** purpose **ranked** — (1) internal agency working doc → (2) client-presentable export (same record, findings reframed as opportunities via an `internal|client` render profile) → (3) enforcement-refresh **suggest-only in v1**; **dual-layer** (Documented + Proposed) every section; **grounded synthesis** — invent brand language, NEVER a product/efficacy/dosage/regulatory claim.
- **D4 REVERSED (ADR in `decisions.md`) — no headless browser.** The original Browserless-over-CDP choice was dropped: visual extraction is best-effort **garnish**, so palette = **DataForSEO `page_screenshot` pixel-dominance (Pillow)** + **scraped-CSS hex** (ScrapeOwl `render_js`); the vibe read = one **Sonnet-vision** call over that screenshot (the existing `qa_visual` pattern). No Chromium, no Browserless, no new dep/key/topology. ⚠️ **The "declared hex from scraped CSS" half is an UNPROVEN hypothesis** — ScrapeOwl returns rendered DOM, so external-stylesheet / `var()` / Tailwind colors may not surface as inline declarations — **gated on a Phase-0 spike** (PRD §10 / acceptance #1): run the real ScrapeOwl + DataForSEO + Pillow path against ≥3 live sites *on the worker* and measure vs a manual eyedropper BEFORE Phase 1 builds on it (the only prototype so far validated the *discarded* computed-styles method on an all-inline fixture).
- **Regulated gate reuses `clients.content_compliance_mode` (`!= 'off'`), NOT a new flag** — a deterministic claim-shape INPUT-filter (check `services/content_compliance.py` first for a reusable detector) + a pre-render **sign-off** gate. Residual "regulated client left at `off`" risk explicitly accepted (owner).
- **Sign-off = status + a second job, not a paused job** (in-process jobs can't pause mid-run): `brand_guide_generate` stops at **`awaiting_signoff`** for regulated clients; an admin / owning-staff approve enqueues a separate **`brand_guide_render`** job (mirrors the GBP Profile Editor domain-status pattern).
- **§4.4 reads the human seed via `resolve_brand_guide_text` / `resolve_icp_text`** (`brand_voice.raw_text` / `detected_icp.raw_text`) — NOT the legacy top-level `brand_guide_text` / `icp_text` columns, which are the empty default for every current client.
- **Data model:** new `brand_guides` table (versioned; `status` incl. `awaiting_signoff`; `edited`; per-profile `storage_path`/`pdf_url`) + the two `async_jobs` types. Migration `writer/supabase/migrations/<ts>_brand_guides.sql` — **not yet written/applied.**
- **Deliberately un-folded (four minor adversarial findings — expected, not regressions):** filter-can-empty-the-input degrade note, edits-lost-on-regenerate semantics, "lightweight editor" scope understatement, vibe-read unfalsifiability.
- **Next:** a build session starting at **Phase 0** — the pure extraction core (`services/brand_guide_extract.py` + fixtures + unit tests) **and the decisive palette-method spike that gates D4.**
