...
- **Observed lead range** (`homeadvisor_lead_range`): Fence → $55–175 ($98 geomean, medium), Solar → $100 (low — single-sourced). The only two manual categories with a genuine same-trade published Service Direct price; confidence is per-category (`_CATEGORY_OBSERVED`), and `observed_range` returns the modal published pair (order-independent, not the first row).
- **Job-value formula** (`job_value_formula`, low): `CPL = weighted_job_value × close(0.42) × margin(0.22)`, **clamped [20, 150]**. `weighted_job_value` (pure, sample-weighted, spec §4 "vertical-weighted not flagship"); 25 categories mapped conservatively by url-slug. **The cap (= observed GC/remodel lead-range high, spec §1) is the load-bearing guardrail** — the raw formula over-shoots high-ticket trades because CPL decouples from job value at the top (roofing's $7,696 job resells at $85–550, not thousands). All three knobs config-calibratable (`leadoff_formula_close_rate`/`_cpl_cap`/`_cpl_floor`).

**Result:** **27 of 45** manual categories re-grounded (18 stay manual). Tiers now 25 high / 28 medium / 54 low. 19 lifted 1.5–3.1× (the high-ticket project trades cluster at the $150 cap); a few grounded DOWN where the manual guess was high (Solar 170→100, building inspector 60→29, cleaning ~$20–23) — grounded > guess, as v1 lowered handyman/carpet. **Deliberate exclusions (nothing fabricated):** moving (spec §5 gap), piano/furniture/upholstery/ponds/fountains/sprinklers/snow (no match), **Tile** (CSV only has grout/repair — a tile installer's value is flooring-class, uncaptured → keep manual), and chimney-rebuild dropped from Chimney services (n=0 outlier).
...
**Config** (`leadoff_cpl_margin_share`, `leadoff_cpc_modifier_*`, `leadoff_monetization_*`) all calibratable. **Verified:** 384 leadoff backend tests green (3 new test files + modifier assertions in the existing ones), ruff (CI select + default) clean, `tsc -b` + `vite build` clean, `LeadOff.tsx` at its pre-existing 4 errors / 1 warning (**zero new**). **Adversarial self-review fix folded in (commit `b377ed3`):** `tryout_rows` computed `leads` but never put it on the row, so the monetization print's PPL-shared was always `None` on the live grade card + tryout rows (board/brief path unaffected — `enrich_grade` sets `est_leads_mo`); fixed by exposing `est_leads_mo`, + hide the monetization block when there's no exclusive value (no-CPL category).
...
**Faithful to the ONE Enigma shape we know works:** it reuses the **outreach module's live-verified** GraphQL `search(searchInput)` query (`outreach/api/services/enigma_graphql.py`, proven against a real eval key) — the single `cardTransactions` connection filtered by `quantityType`+`period`, the `enigmaId: null`-on-a-real-match gotcha — rather than guessing endpoints. Pulls `card_revenue_amount` (1m/3m/12m/24m) + a **best-effort** `card_transactions_count` alias for avg-ticket (an unknown slug returns empty, never errors; the raw JSONL surfaces the real slug → set `--count-quantity`). Trend is a real trailing-YoY when 12m+24m are present, else a labelled run-rate proxy (never presented as a YoY).

**Ground truth:** **bucket A is pre-filled** with the 5 real Little Rock, AR water-damage competitors LeadOff already holds (names/addresses/domains from `competitor_locations`, incl. the generic/aggregator worst-case rows); **bucket B (revenue anchors) + the positive control are `#`-placeholder rows the OWNER fills** (the two numbers only they have). Deterministic verdicts + the §5.4 matrix are unit-tested (`tests/test_enigma_coverage_pilot.py`, 27 cases — a sign-flipped threshold would mislead the go/no-go); §5.3 avg-ticket plausibility is a first-pass AUTO flag with an authoritative `plausible_human` column.

**Verified in-session:** 27 pytest cases green (venv: pytest+httpx), `--dry-run` prints a well-formed GraphQL doc + correct variables for the first bucket-A row, no-key path exits 2, ruff (CI `select E9,F63,F7,F82` **and** default config) clean, not on the mypy allowlist. **Still cannot RUN from a Claude Code session** — no trial key + the sandbox is egress-blocked from Enigma. **Owner action:** fill the CSV's bucket-B + control rows, `export ENIGMA_API_KEY=… ENIGMA_GRAPHQL_URL=…`, then `python scripts/enigma_coverage_pilot.py` (owner machine or a Railway shell); paste the printed scorecard into plan §8. Working hypothesis unchanged: growth is the salvageable use, lead-value likely fails §5.3 (insurance-undercount) → fall back to won-client close data via `leadoff_calibration`.

**OPEN ITEMS / next (LeadOff on-demand), updated:** (1) apply the 4-category board-add runbook (owner's scanner machine); (2) sub-10k geocode step (NOT built); (3) ~~Enigma coverage-pilot harness~~ — **BUILT this session** (owner runs it); (4) drop the demand gate to 10 on the ≥30k board (optional, scanner machine).

---

## ⏩ Update — 2026-09-18 · **LeadOff — Scout button on Tryout rows (BUILT + MERGED, PR [#1223](https://github.com/kssabraw/ar-tools/pull/1223), squash `de26347`).**

The open item flagged in the merged grader entries below ("a scout button on Tryout rows — scout is on the single-service grade card only") — now merged to `main` (all CI green: platform-api tests + lint/typecheck + Netlify). Now every **completed tryout row** (a city × one of the ~100 catalog categories) carries a **"Scout"** button beside its "Map" button — deepening THAT market (competitor referring domains, review velocity + momentum, demand trend, brand footprint) without a board row, the exact off-board analogue of scout-from-grade (#1212).
...
## ⏩ Update — 2026-09-18 · **LeadOff — Board-search "Grade it live?" handoff (MERGED, #1217).**
...
## ⏩ Update — 2026-09-18 · **LeadOff grader — grade the LITERAL keyword, not the catalog category (fix, MERGED #1217).**
...
## ⏩ Update — 2026-09-18 · **LeadOff `grade-all` bulk sweep ("Rank cities") — BUILT + MERGED (#1217). Scout-from-grade (PR #1212) merged earlier on `main`.**
...
**Grader — how it resolves a lookup, cheapest path first (`services/leadoff_grade.py`):**
1. **Board-first (FREE)** — exact city×category already on `leadoff_board` → its enriched `get_market_brief` grade.
...
**Design calls:** the live path does **NOT** apply the `vol≥20` demand gate — it grades the requested cell regardless and *surfaces* `thin_demand` (the gate only ever bounded PRECOMPUTE cost). Off-catalog services still grade live; only the CPL falls back to `leadoff_finder_default_lead_value`, flagged `cpl_default=True`. Staff-gated + per-user daily-budget guarded (`check_budget`/`record_spend` action `'grade'`), like Tryout.

**Grader shipped (PR #1204, merged, squash `deb55f17`):** migration `20260918120000_leadoff_grades.sql` **applied live** (`leadoff_grades` + RLS; `'grade'` on `leadoff_spend`; `'leadoff_grade'` on `async_jobs`, rebuilt from the verified live constraint); `services/leadoff_grade.py`; `POST /leadoff/grade` + `GET /leadoff/grade/{id}`; `job_worker` dispatch; **frontend "Grade a market" tab** on `pages/LeadOff.tsx` (`GradeView`); tests `tests/test_leadoff_grade.py`.

**Scout-from-grade shipped (PR #1212, GREEN + mergeable draft, branch `claude/leadoff-grade-scout`, head `a0815242`):** a **"Scout this market"** button on the grade card deepens a graded OFF-BOARD market (competitor referring domains, review velocity + momentum, demand trend, brand footprint). The existing scout was board-only (`scout_market_state` needs a `leadoff_board` row + `serp_top5`); the fix injects the graded market's stored top-5 into the same `leadoff_scout` job (`grade_id` in the payload → the off-board path) and stores the enrichment back on `leadoff_grades.scout` (migration `20260918160000` **applied live**). Reuses the `leadoff_scout` job + `'scout'` spend action (no new types). Gated to **on-catalog** grades (scout keys on `category_id`). `scout_market_state` gains optional injected `market`/`comps` (board path byte-identical); `leadoff.scout_enrichment` reads the Pass-2 caches for the grade's competitors. Tests `+7` → full leadoff suite **343 green**; ruff clean; `tsc`/`vite build` clean. **CI all green** (pytest/ruff/mypy/Netlify). **Owner has NOT merged** (asked to build, not merge — #1204 was merged by explicit request).

**Sort-by-service — what works TODAY (verified live 2026-09-18):** a user CAN rank cities for a service on the **Board** tab (filter by the category / smart-search, sort by Opportunity / Expected value / Profit / Payback). Scope: **scanned categories only (~100 GBP services) AND ≥30k cities that cleared that service's demand gate.** For HVAC (`hvac_contractor`) that's **723 cities, free + instant**. Sub-30k cities and off-catalog services are NOT in that cross-city sort — they need the per-city grader (one at a time) or a paid sweep. The `find-cities`/`city-finder` surfaces answer the same "which cities for service X" question.

**Live city coverage (the grader's reach, `market_scanner.cities`):** 4,682 US places ≥10k pop; **3,993 have a Google Ads location code** (the gradeable set — demand pull needs it). 15k–30k = **1,681** (1,406 w/ loc code), 10k–15k = **1,273**, none on the board. Floor is **10k** (below → `city_not_found`, needs a geocode step). So the 15k–30k tier is fully gradeable on demand today, just not precomputed.

**Cost to grade all cities for one service (e.g. an HVAC lead-gen sort):** ~$0.06/city (1 keyword task ~$0.05 — can't batch across cities, one location_code per task — + 1 Maps SERP ~$0.004). All 3,993 gradeable ≈ **~$240**; minus HVAC's 723 already-free board rows ≈ **~$195**. The per-user daily budget guard defaults to **$5/day** (an app safety, separate from DataForSEO which the owner confirmed has NO $500/day cap) so a sweep needs it raised; there is **no `grade-all` endpoint yet** (grader is 1-at-a-time; city-finder caps 300/run).

**OPEN ITEMS / next (LeadOff on-demand):**
...
## ⏩ Update — 2026-09-18 · **LeadOff — Enigma card-transaction data pilot: NOT RUN (blocked on a trial key + egress; harness NOT built yet).**

The "better data for pricing/profit" thread. Plan: **`docs/modules/leadoff-enigma-pilot-plan-v1_0.md`** (PROPOSED go/no-go, owner scope 2026-08-28). Enigma would attach a **real card-transaction revenue signal** to the incumbents LeadOff already identifies — the one economic signal Google/DataForSEO can't give — for two owner-chosen uses: **incumbent growth signal** + **lead-value calibration** (grounding the manual `lead_values` CPLs in real average-ticket dollars → better expected value / profit).

**Why it's coverage-first + the working hypothesis:** home-service SABs (LeadOff's categories) are Enigma's structural weak spot — heavily insurance/invoice/check/ACH-paid (a water-damage job is billed to an insurer, not swiped) + address-less/generic-named (hard to match). Doc's hypothesis: **growth is the salvageable use; lead-value calibration is the risky one** (card data undercounts → biases CPL down), and the better lead-value source may be **the agency's own won-client close data** via the existing `leadoff_calibration` loop.

**State checked live 2026-09-18:** `ENIGMA_API_KEY` NOT set in this env; no Enigma key/host/HTTP client anywhere in platform-api (the only Enigma code is the **outreach** module's ordering/budget side — `outreach.py`, `outreach_enigma_*` settings — not a reusable LeadOff client); no LeadOff pilot script; the sandbox has no egress to Enigma's API. **So the pilot cannot run from a Claude Code session** — it needs (a) an Enigma trial/eval key (a human signup step) and (b) to run where there's egress (owner's machine or a Railway shell with the key set).

**Owner asked (2026-09-18) to "run the coverage pilot on a trial key"** — blocked as above. **Next (agreed prep, no vendor commit, no new dep):** build a runnable `scripts/enigma_coverage_pilot.py` (reads `ENIGMA_API_KEY` from env, takes a ~12 home-service-SAB ground-truth CSV, calls Enigma's Small Business API, prints the coverage + dollar-plausibility scorecard vs the plan's §5 go/no-go thresholds; refuses to run without a key) + assemble the ~12-business ground-truth set. Then the owner runs it in one command once they have a trial key. **NOT started** — awaiting the go-ahead to build the harness (or the owner's decision to skip Enigma for lead-value and wire the won-client-close-data calibration path instead).
...
**A — `never_use_terms` coverage-exclusion (§13 #1).** The `_compute_serp_signal_coverage` engine scored the page against the forbidden target keyword (e.g. "retatrutide" — 34 kw + 66 bold shortfall) and its recommendations literally said "add retatrutide", which voice enforcement then strips every pass. Pure `voice_card.strip_forbidden(items, never_use_terms, text_of)` (unit-tested — plain-string + dict `text_of` + word-boundary-not-substring + None-tolerant) drops any coverage target matching a `never_use_terms` word-boundary regex (`build_term_regex`). `_compute_serp_signal_coverage` gained an optional `never_use_terms=` param and filters the ranked-keyword zones / entities / quadgrams / bold-keyword targets through it before scoring; threaded from `(voice_card or {}).get("never_use_terms")` at all 5 call sites. **Composite-neutral by construction:** a forbidden term is never *found* on a voice-compliant page, so removing it from the target set leaves `found/target` identical — only the phantom "missing" instruction is deleted. Safe even though this engine IS in the composite.

**B — `<title>`-in-scorer extraction parity (§13 #3).** The `organic_ranking` engine reported it couldn't see the title ("Title tag content not provided…"), so the qualitative engines under-scrutinized the single heaviest element the deterministic engine already measures. `_TITLE_TAG_RE` + `_extract_title_tag(page_html)` (regex → strip inner tags → collapse whitespace → cap 300) + `_title_block(page_title)` prepend a `PAGE TITLE (<title> tag): …` line to the content block of all three page-scorer prompt builders (`page_title` param defaulting `""`), passed `page_title=_extract_title_tag(page_html)` at all 5 call sites.

**C — empty-entity diagnosis logging (§13 #2).** `entity_detail: []` on an entity-rich SERP was ambiguous: genuinely no entities, or the `GOOGLE_NLP_MIN_SALIENCE=0.40` floor threw them all away? `get_google_entities` now builds its page-spread `candidates` **before** the salience cut, logs the salience distribution + a `{kept}/{candidates} kept` line, and on zero survivors logs `{distinct} distinct entities across {pages} pages, {raw} raw mentions` (mirrors the calibration logging TextRazor already had). Output shape unchanged — makes the cause observable in prod so the floor can be tuned from real data. **Diagnosis-only; needs a live `/analyze` to read (post-deploy).**

**D — scheduled site-claim-index refresh.** The P1 grounding index is built lazily per client on a 30-day TTL, so a **stale** index silently suppresses gain until the next score request rebuilds it. Pure `site_claim_index.select_stale(rows, ttl_days, now)` (reuses `is_fresh`; missing `fetched_at` → stale; order-preserving) + async `refresh_stale_indexes(now)` — double-gated on `topic_vector_gain_enabled` AND the new `site_claim_index_refresh_enabled` (default True), bounded to `site_claim_index_refresh_max_per_tick` (default 5) oldest-first, rebuilding each stale index against the client's **CURRENT** `website_url` (skips archived) via the same single-flighted `_build_coalesced` the score path uses. Wired into the shared `gsc_scheduler` daily block via `_safe_async("site_claim_index_refresh", …)` (once/day, idempotent — a re-run finds them fresh). **Refreshes only EXISTING index rows** (clients already using the feature); never pre-builds for the whole roster (no wasted crawl budget).

**Tests/lint:** `nlp-api/tests/test_voice_card.py` **78 passed** (5 `strip_forbidden` cases), `platform-api/tests/test_site_claim_index.py` **17 passed** (5 new: `select_stale` filter/order + disabled-TTL, disabled-flag gate, rebuild-stale-non-archived-with-current-website, cap-zero noop). `py_compile` clean on every touched file; ruff `E9,F63,F7,F82` clean. Card A's core is tested via the reusable pure `strip_forbidden` (nlp `main.py` isn't importable in-sandbox — slowapi/fastapi absent); B/C's `main`-local helpers are `py_compile`-verified + regex-sanity-checked.

**⚠️ Live validation post-deploy.** Merging redeploys all three services. C's entity-diagnosis logging + D's actual crawl land on the deployed services (sandbox egress-blocked from ScrapeOwl/nlp/Gemini; nlp `main` not importable). A/B are prompt/target changes verifiable by re-scoring a page whose client has `never_use_terms` set. **The topic-vector module's "not built (beyond v1)" list is now empty.**

## ⏩ Update — 2026-09-16 · **Topic-Vector — BLOG REOPT GAIN COACHING — BUILT (report-only)** — PR [#1175](https://github.com/kssabraw/ar-tools/pull/1175) (draft)
...
**How it travels (Approach A — render in nlp, single-sourced).**
- **nlp renders it once at SCORE time.** `/score-blog-page` now attaches `topic_vector.render_gain_guidance(measure, site_claim_index, page_text)` onto the report-only `topic_vector.gain_guidance` field (platform-api can't import the pure nlp renderer, so nlp is the single source). "" when nothing actionable / measure unavailable → byte-identical downstream. It stays a string on the free-form `Optional[dict]` `topic_vector` field — never in `scores` (survives serialization; the ReoptAction/topic_vector lesson).
- **A blog reopt reuses that string (NO second nlp call).** Two entry points, both covered:
  1. **In-place reopt** — `blog_page_score.reoptimize_run` → new `_reopt_gain_guidance(run_id)` reads the run's latest `blog_score` output's `topic_vector.gain_guidance` and threads it via `_build_writer_payload(source_gain_guidance=…)`.
  2. **Reopt-of-existing (URL/paste)** — orchestrator blog **Stage B′** reads `src_score.topic_vector.gain_guidance` off the `blog_source_score` and passes it to `_build_writer_payload`. The **Fanout** blog reopt spawns a Stage-B′ run, so it inherits this automatically (no separate wiring).
- **The carrier:** `orchestrator._build_writer_payload` gained `source_gain_guidance` → a new **input-only** `WriterRequest.reopt_gain_guidance` (no output-schema bump — mirrors `mode`/`prior_sections`/`deficiencies`). In the pipeline writer, the pure `compose_reopt_notes` (`modules/writer/reopt.py`) folds it into the section/intro/conclusion steering **alongside** `user_notes` + the reopt deficiency directive, but **EXCLUDES it from the notes-landed QA** (`notes_for_qa`) — it's advisory "improve where it fits", never graded as an unmet user directive — and it is **NEVER a `deficiencies` entry** (report-only, composite weight 0; §11.1 / Content-Gap precedent).

**Governance held (same as P0/P1/P2):** report-only (composite weight 0, never in `scores`, never a deficiency, never blocks — the blog composite/deficiencies are byte-identical); best-effort (no GEMINI key / thin-or-absent site index / empty measure → "" → the writer payload + prompt are byte-identical to today, never raises); the pushed-facts half rides the inherited `_COACHABLE_FACT_TYPES = {coa, purity, storage_temp}` allowlist (the #1171 garbage-fact fix) + the `never_use_terms` scrub — both verbatim in `render_gain_guidance`, not re-implemented; deterministic engines untouched; **no new nlp blog-rewrite endpoint.**

**Tests (offline; sandbox egress-blocked from nlp/Gemini/DataForSEO):** `nlp-api/tests/test_topic_vector.py` (the score-time attach contract — non-empty when actionable, "" when not, report-only), `pipeline-api/tests/test_writer_reopt.py` (`compose_reopt_notes` — gain rides in section notes not QA, empty → byte-identical, never a deficiency), `platform-api/tests/test_blog_page_score.py` (`_reopt_gain_guidance` read + `reoptimize_run` threads it, omits key when empty) + `test_orchestrator_service_page.py` (`_build_writer_payload` gain param). ruff (E9,F63,F7,F82) clean on every touched file. Verified green locally: pipeline writer suite (`test_writer_reopt`/`test_writer_pipeline`/`test_writer_user_notes`/`test_writer_format_qa`), platform (`test_blog_page_score`/`test_orchestrator_service_page`/`test_service_page_score`/`test_score_external`/`test_fanout_reoptimize`), nlp (`test_topic_vector`). Pre-existing sandbox missing-dep failures (spacy in `test_brief_pipeline_v2`) are environmental, not from this change.

**⚠️ Live validation is post-deploy.** Merging redeploys the `nlp` service. To confirm the live path: score a client blog article that has a populated `site_claim_index` → the `blog_score` output should carry a non-empty `topic_vector.gain_guidance`; then reoptimize that run and confirm the pipeline writer received `reopt_gain_guidance` (in-request logs / the resulting article covering the coached subtopics). **Not touched (separate cards, each needs its own owner scope):** the §13 scorer fixes (`never_use_terms` coverage-exclusion + title-in-scorer, empty-entity diagnosis) and a standalone scheduled site-claim-index refresh job.

## ⏩ Update — 2026-09-16 · **Topic-Vector — P2 EMOTIONAL-ARC RUBRIC — BUILT (report-only) + MERGED (PR [#1173](https://github.com/kssabraw/ar-tools/pull/1173), squash `c34d650`)**

The final phase of the topic-vector module. Design authority `docs/modules/topic-vector-information-gain-plan-v1_0.md` (§10a the arc / §12 P2). **v1 is now COMPLETE — P0 + P1 + P2 built + merged to `main`.** Merged `c34d650` — this redeploys the `nlp` Railway service, so live LLM-path validation is post-deploy (see the ⚠️ line below). CI green on the head (`nlp-api tests` ✅ / Netlify ✅).

**What it is.** An LLM RUBRIC dimension — NOT an embedding/cosine signal — checking whether a page moves THIS client's buyer from an anxious "before" to a confident "after". `before` = the voice card's `audience_pain_points` + `audience_objections` + `audience_triggers`; `after` = `audience_motivations` (+ objections answered). The SUBORDINATE TAIL of the model (MCS-first): report-only, **composite weight 0**, never in `scores`, never in the cosine (affect isn't embeddable).

**No new payload.** The audience fields ride on the voice card, which is ALREADY resolved inside every scorer via `main.py::_resolve_voice_card(client, body)` (from the `brand_voice`/`detected_icp`/`voice_card` the request body already carries — platform-api sends the cached `voice_card` on every score). So P2 reads the card that's already there — **nlp-only, no platform-api change, no migration.**

**As built.**
- **Pure logic** (`nlp-api/topic_vector.py`): `build_arc_states` (before/after lists from the audience fields), `has_arc_inputs`, `suppressed_arc(reason)`, `build_arc_prompt`, the `ARC_TOOL` forced-tool schema + `ARC_SYSTEM`, and `sanitize_arc` — which **drops unevidenced verdicts** (a "yes" with no verbatim page quote is flipped to negative, mirroring the vibe_read sanitize) and **scrubs every arc string of `never_use_terms`** (a forbidden word never appears in any verdict/evidence/rationale/concern — §10b non-negotiable).
- **The one Haiku forced-tool call** (`main.py::_measure_emotional_arc`) — best-effort, never raises. Attached in `_measure_topic_vector` (refactored: the embedding body became `_measure_topic_vector_embeddings`; the wrapper runs it, then attaches the arc as an **`emotional_arc`** sub-object when the caller passes `voice_card` + `client`). The arc is independent of the embedding half, so it attaches even with no `GEMINI_API_KEY`.
- **Wired onto the MONEY-PAGE scorers** (`/score-page` local+service, `/score-ecommerce-page`) — the reopt endpoints (final-response + coaching) call the wrapper WITHOUT the arc kwargs, so they're byte-identical (report-only, no coaching — like P0 centering). **Blog + Fanout mass posts are DELIBERATELY EXCLUDED** (owner ruling 2026-09-16): that content is informational (TOFU/MOFU) and the arc's before→after is a buyer/BOFU transition from the voice card's purchase-decision audience fields, so judging an explainer by it would penalise good informational copy — consistent with §10a "one page = one awareness stage." `/score-blog-page` still runs centering/coverage/gain, just not the arc (it calls `_measure_topic_vector` without `voice_card`/`client`).
- **Suppressed (never scored 0):** no voice card / no audience fields → `no_audience_fields`; LLM/parse failure → `arc_failed`; flag off → `arc_disabled`.
- **Config on nlp:** `TOPIC_VECTOR_ARC_ENABLED` (default True), `TOPIC_VECTOR_ARC_MODEL` (Haiku `claude-haiku-4-5-20251001`, mirrors `VOICE_LOCALIZE_MODEL`/`PAGE_SPEC_AUDIT_MODEL`), `TOPIC_VECTOR_ARC_MAX_TOKENS` (1200). Arc token cost is logged + carried inside the `emotional_arc` sub-object (not folded into the handler's `token_usage`, so the score response contract is unchanged).

**Governance held (same as P0/P1):** `_compute_serp_signal_coverage` byte-identical (the arc is a separate pass beside it); composite weight 0, never in `scores`; the new `emotional_arc` rides on the already-declared report-only `topic_vector` response field (no response_model change needed — it's a free-form `Optional[dict]`; verified no downstream consumer parses that field's contents — platform-api reads only the `topic_vector_gain_enabled` flag, frontend reads none). NOT wired into any reopt-coaching loop (owner scope not given).

**Adversarial-review hardening (folded into the merge):** a forced-tool response with NO tool_use block is suppressed (`arc_failed`), never a fabricated `available:0`; `_arc_clean_str` scrubs `never_use_terms` on the FULL string before capping, so a term straddling the cap can't leave a fragment; `_arc_list` guards a non-list audience field (yields `[]`, never char-iterates).

**Tests/lint:** `nlp-api/tests/test_topic_vector.py` **45 passed** (9 P2 cases: state assembly, prompt carries audience + page text, unevidenced-verdict drop, no-audience→suppressed-not-0, score clamp + garbage-safe, forbidden-term-never-surfaced, scrub-before-cap, non-list-field guard); `py_compile` + `import main` clean (import blocked in-sandbox only by the `nltk_data` runtime download, far above the P2 code); ruff `E9,F63,F7,F82,F821` clean on `main.py`/`topic_vector.py`/`test_topic_vector.py`. **No platform-api diff** (P2 is nlp-only).

**⚠️ LIVE VALIDATION OF THE HAIKU RUBRIC PATH IS POST-DEPLOY** (sandbox egress-blocked from the private nlp service; the pure logic runs offline). Once deployed: run a **local / ecommerce** score on a client whose voice card carries audience fields → confirm `topic_vector.emotional_arc` populates with `available:true`, a 0–100 `score`, evidenced transitions (`resolved_count`/`transition_count`), and `composite_weight: 0`; a client with no audience fields shows `{available:false, reason:"no_audience_fields"}` (suppressed, not 0); and a **blog** score carries **no** `emotional_arc` (deliberately excluded). **Do NOT start the other remaining cards** (a distinct problem-aware arc for TOFU/MOFU blog content · blog reopt coaching · the §13 scorer fixes · a scheduled site-claim-index refresh) — each needs its own owner scope call.

## ⏩ Update — 2026-09-16 · **Topic-Vector — P1 reopt-COACHING garbage-fact FIX + gain coaching wired into the LOCAL SEO reopt loop — MERGED (PR [#1171](https://github.com/kssabraw/ar-tools/pull/1171), squash `f1ea9b6`)**
...
**FINDING — the coaching pushed 6 bad facts of 8.** For a retatrutide reopt the block would tell the writer to "state" a garbage **`price: 0.00 USD`** (an empty-cart chrome artifact the price regex scraped from `/blog/`) + five cross-product **`size`** facts (25/4/5/6/75 mg — semaglutide dosage-comparison numbers from a blog, not this product's 10mg/30mg vials). Only `coa: present` + `storage_temp: -20 °C` were defensible. **Root cause:** the site-claim index is CLIENT-level (whole site), but coaching PUSHES a fact onto ONE page — for a multi-product client the per-product / per-compound / transactional facts belong to another page.

**FIX (this branch, two parts).** (1) **Extraction** (`site_claim_index.extract_facts`): drop `$0`/`$0.00` prices (empty-cart artifacts, never a real price; also stops a `0.00` claim false-grounding via `_site_fact_values`). (2) **Coaching** (`topic_vector.render_gain_guidance`): push only SITE-INVARIANT fact types — `_COACHABLE_FACT_TYPES = {coa, purity, storage_temp}` (the quality/handling facts a vendor asserts site-wide); never `price`/`size`/`cas`/`molecular_weight`/`molecular_formula` (identity/commerce, page-specific). The scored-gain GROUNDING path is untouched (it corroborates the page's OWN claim — safe for every type), and the subtopic-gap coaching (keyword-anchored) is unchanged. Offline re-run on the same real Nova data → block now = the real subtopic + `coa: present` + `storage_temp: -20/-80 °C`; garbage + cross-product sizes gone.

**Not a new defect:** the age-gate chrome ("I acknowledge that I am age 21 or older.") still scoring `realized` in the 05:58 run is the **#1169 `_CHROME_CLAIM_RE` fix not-yet-deployed** (merge-queue lag), not a new issue.

**Tests/lint:** nlp `test_topic_vector.py` **36** (existing CAS-coaching test updated to storage/purity; new allowlist-exclusion test), full nlp suite **321 passed**; platform `test_site_claim_index.py` **12** (new zero-price-drop test) + `test_ecommerce.py`/`test_local_seo_service.py` green (109 together); full platform suite **6375 passed** (the only failures/errors are pre-existing sandbox missing-dep issues — confirmed via `git stash`); ruff E9/F63/F7/F82 clean; `_CHROME_CLAIM_RE` cross-file mirror untouched (the fix is in the price loop, not the chrome regex). **Post-deploy:** delete Nova's `site_claim_index` row (or wait the 30-day TTL) to force a clean re-crawl so the stale `price: 0.00` clears from the grounding fast-path.

**ALSO in this PR (owner-greenlit next phase): gain COACHING wired into the LOCAL SEO reopt loop.** Previously only `/reoptimize-ecommerce-page` coached gain; the Local SEO/service/blog measure was report-only on the SCORERS. Now nlp `/reoptimize-page` (the Local SEO location/service reopt endpoint) coaches it too — `ReoptimizePageRequest` gained `site_claim_index`; the handler runs `_measure_topic_vector(existing_html, keyword, serp_analysis_dict, site_claim_index, include_gain=False)` → `render_gain_guidance` → injects the guidance block into the first rewrite prompt (beside `writer_notes`), best-effort. Platform: `local_seo_service.reoptimize_page` sends `site_claim_index.resolve_index_for_request(client)` in the payload (covers the in-tool reopt AND `reoptimize_url`/bulk, which route through it). Report-only (composite weight 0); the same site-invariant allowlist protects it. Test `test_reoptimize_page_threads_site_claim_index_to_nlp`; local_seo suite 66, nlp suite 321 green. **Not wired:** blog reopt coaching (pipeline-api) — a separate card. **REMAINING PHASES (need their own scope call):** P2 emotional-arc rubric · the §13 scorer fixes (`never_use_terms` coverage-exclusion + title-in-scorer, empty-entity diagnosis) · a scheduled site-claim-index refresh job.

## ⏩ Update — 2026-09-16 · **Topic-Vector — P1 gain path LIVE-VALIDATED + measure EXTENDED to Local SEO / service / blog scorers**

Two things on top of the merged P1 (#1164), on branch `claude/topic-vector-information-gain-iti9fp`. Design authority `docs/modules/topic-vector-information-gain-plan-v1_0.md`.

**1) P1 gain path live-validated** (the pending post-deploy check). After the owner merged #1164, ran a production-shape `ecommerce_action` score job on **Nova "buy retatrutide"** through the live PLATFORM worker (sandbox egress-blocked from nlp/DFS/Gemini, so validated the way P0 was — direct `async_jobs` insert, result read back via Supabase):
- **site claim index built live**: 7 URLs, source `sitemap_truncated`, **14 typed facts** (real vial sizes 1–75 mg / COA present / storage −20/−80/8 °C — clean) + **60 claim phrases** — a real, non-thin row.
- **gain block populated** (`information_gain.available:true`, `score:80`, `composite_weight:0`).
- **ungrounded-but-on-vector claim → 0 + flagged** ✅ (a "View the 10mg/30mg Certificate Of Analysis … Size Choose a" nav fragment → `ungrounded_novelty`, `realized_gain:false`, in `ungrounded_claims`).
- **name-agnostic grounding confirmed** ✅ ("GLP-3RT … targets GIP/GLP-1/glucagon" grounded `site_cosine 0.84` vs the site's own coded-name claims; the "Multi-Receptor Activation" claim scored `realized_gain:true`, rare `0.71`<0.72, grounded `0.82`).
- **chunked Gemini `batchEmbedContents` ran live** (the review-hardening chunking works end-to-end). Centering `0.7707` (consistent with P0's ~0.747). Grounding **separates real vs fabricated** — the gate the task set. Floors roughly calibrated (`GAIN_RARITY_CEILING 0.72` / `GAIN_GROUNDING_FLOOR 0.78` sit inside gemini-embedding-2's compressed band; not the bottleneck).

**Defect found + FIXED (this branch):** site-chrome carrying a stray number (the age-gate **"I acknowledge that I am age 21 or older."**, "25% Off" discount popups, "N min read" blog-index blurbs) passed the fact-signal gate and — because the identical chrome sits in BOTH the page-claim and site-claim corpora — **self-grounded into a false realized gain** (the age-gate scored `realized_gain:true` at `site_cosine 1.0`, inflating the score). A narrow shared **`_CHROME_CLAIM_RE`** denylist now drops it in **both** claim extractors (`nlp-api/topic_vector.py::extract_page_claims` + `platform-api/services/site_claim_index.py::extract_claims` — mirrored, each unit-tested). Typed-fact extraction was already clean, so the reopt coaching (typed facts + inverse-gain gap, not raw claims) is unaffected. A live reopt-coaching confirmation is deferred until the merge queue stops redeploying nlp every few minutes (the score job survived one such mid-run redeploy — the nlp POST retried onto the new container — a decent resilience signal).

**2) Measure extended to the Local SEO / service / blog scorers** (report-only, the owner-chosen next phase). Centering + coverage + inverse gap + scored Information Gain now run BESIDE the composite on **`/score-page`** (Local SEO + service, both geo modes) and **`/score-blog-page`** (blog):
- nlp: `ScorePageRequest`/`BlogScoreRequest` += `site_claim_index`; `ScorePageResponse`/`BlogScoreResponse` += report-only `topic_vector` (declared on the response models — else FastAPI strips it, the ReoptAction lesson); each handler calls `_measure_topic_vector(page_html, keyword, serp_analysis_dict, body.site_claim_index)`.
- platform: `site_claim_index.resolve_index_for_request(client)` threaded from `local_seo_service.score_page`, `service_page_score` (all 3 score sites), `blog_page_score` (both). SERP-rank tiering was already shared (`_run_serp_analysis` emits `competitor_heading_tiers` for every scorer), so wiring, not new SERP work.
- **efficiency:** `measure()` extracts + embeds the gain claims only when the index is usable (`want_gain = include_gain and not _index_is_thin(...)`), so a `/score-page` consumer that passes no index (content-gap, the strategist `audit_page`, a raw score) runs the P0 measure with no wasted page-claim embeddings — identical output (gain still suppressed, same reason).
- Non-negotiables held: `_compute_serp_signal_coverage` byte-identical (measure is a separate async pass, composite weight 0); gated on `GEMINI_API_KEY`; thin/absent index → gain SUPPRESSED not 0.

**Tests/lint:** nlp `test_topic_vector.py` 35 (chrome-drop + no-index-skips-gain-embeddings added), full nlp suite **320 passed**; platform `test_site_claim_index.py` 11 (chrome-drop added) + `test_local_seo_service.py`/`test_ecommerce.py` green; ruff (E9/F63/F7/F82) clean; no import cycle. **Not built (P2):** the emotional-arc rubric dimension. Separate cards (don't build without a green-light): the §13 fixes (`never_use_terms` coverage-exclusion + title-in-scorer, empty-entity diagnosis) + a scheduled site-claim-index refresh job (today lazy per-client per 30-day TTL). **MERGED as PR [#1169](https://github.com/kssabraw/ar-tools/pull/1169) (squash `1034145`); all CI green (nlp-api tests / platform-api tests / lint-typecheck / Netlify preview).** Live reopt-*coaching* confirmation still deferred (deferred, not done) until nlp stops redeploying every few minutes.

## ⏩ Update — 2026-09-16 · **Topic-Vector Centering + Information Gain — P1 (site claim index + scored Information Gain) — BUILT, DRAFT PR [#1164](https://github.com/kssabraw/ar-tools/pull/1164)**

P1 of the topic-vector module (design authority `docs/modules/topic-vector-information-gain-plan-v1_0.md` §6 Information Gain / §7 the cross-service contract / §10 the anti-fabrication rule), building on the merged P0 (#1156). **Still report-only + ships dark behind `topic_vector_gain_enabled` (default True, but the whole measure is gated on `GEMINI_API_KEY` and composite weight 0, so it never touches a score).**

**Task 1 — the pending P0 live validation ran first (§14), reported before P1 was built.** Direct `ecommerce_action` score jobs on the **deployed** nlp (a production-shape `async_jobs` insert read back through Supabase — the sandbox is egress-blocked from the private nlp + DataForSEO): §14 acceptance #1 **MET** — the Nova "buy retatrutide" vendor-trust-drift page centred **0.747** vs the competitor bluum PDP **0.845** (~0.10 separation; centering is **orthogonal to the composite** — bluum's SEO composite 52.0 < Nova's 54.8). Two findings drove a floor **calibration**: gemini-embedding-2 cosines run high + compressed (per-subtopic 0.67–0.81, no clean gap) so `CENTERING_FLOOR` 0.45→**0.60** and `COVERAGE_FLOOR` 0.55→**0.70** (the "relative fallback" was tried then **removed** — it over-fired on well-covered pages); and today's live "buy retatrutide" SERP is editorial/listicle-heavy, so its consensus subtopics are vendor-comparison/where-to-buy/FDA-risk, not the mechanism cluster the plan's origin analysis expected.

**Built (P1):**
- **Site claim index** — the per-client grounding corpus. `site_claim_index` table (migration `20260916190000_site_claim_index.sql`, **applied live**) + `writer/platform-api/services/site_claim_index.py`: deterministic (no LLM) typed-fact (price/purity/CAS/MW/formula/storage/size/COA) + claim-phrase (fact-bearing sentence) extraction from the client's OWN site, discovered via `site_page_index.discover_site_urls` → scraped via `website_scraper.scrapeowl_fetch` → merged/deduped → cached (mirrors `ecommerce_facts_cache`, incl. the `"now()"` timestamp pattern). `resolve_index_for_request(client)` is the single entry point: cache-first, build-on-miss, best-effort, gated on the flag, **never raises**.
- **platform→nlp contract** — `ecommerce_service.score_page` + `reoptimize_from` resolve the index and pass it in the nlp request body (`site_claim_index` on `EcommerceScoreRequest`/`ReoptimizeEcommerceRequest`), exactly like `serp_analysis`/`researched_facts`; **nlp has no DB and never crawls**.
- **Scored Information Gain** (`topic_vector.py`) — a page claim counts as gain only if **on-vector** (clears the centering floor) AND **rare in the top-10** (its predicate embedding is far from every competitor SUBTOPIC vector — name-agnostic, reuses the coverage embeddings) AND **site-grounded** (embeds near a site-claim phrase OR states a distinctive typed value the site also asserts). **Ungrounded novelty scores ZERO and is FLAGGED** (the anti-fabrication guard). **Composite weight 0** (never enters `scores`; `_compute_serp_signal_coverage` byte-identical). A **thin/absent** index **suppresses** the score ("not measured"), never 0. Coached into the reopt loop as a guidance block (under-served on-vector subtopics + verifiable facts the client's own site asserts that the page omits — never an invented addition).
- **Review-hardening (adversarial pass, same PR):** (1) **`_gemini_embed` is chunked** at `GEMINI_EMBED_MAX_BATCH` (100) — P1's measure batches page + centroid + subtopics + sections + page-claims + site-claims (up to ~144), which would have exceeded Gemini's per-call `batchEmbedContents` cap and silently skipped **both** the new gain **and** the already-working P0 centering/coverage on content-rich pages (MCS ~13 / P0 ~59 never reach it; chunking is order-preserving, ≤cap = one call, byte-identical). (2) The crawl is **parallelized** under a bounded semaphore (`site_claim_index_scrape_concurrency`=5 — was serial, up to ~200s worst case in the worker) and **single-flighted** per client (`_inflight_builds` — concurrent bulk-lane misses for one client coalesce onto one crawl). (3) The reopt **coaching** measure passes `include_gain=False` — it uses only the inverse-gain gap + deterministic missing-facts, so it skips the scored-gain extraction + ~85 page/site-claim embeddings on the page it's about to rewrite; the final report keeps full gain.

**Config:** `topic_vector_gain_enabled` (True) + `site_claim_index_days` (30) / `_max_pages` (8) / `_max_facts` (40) / `_max_claims` (60) / `_scrape_timeout` (25) / `_scrape_concurrency` (5) — platform; `TOPIC_VECTOR_GAIN_RARITY_CEILING` (0.72) / `_GROUNDING_FLOOR` (0.78) / `_TARGET` (3) / `_MIN_SITE_CLAIMS` (3) + `GEMINI_EMBED_MAX_BATCH` (100) — nlp.

**Tests:** 33 nlp `test_topic_vector.py` (incl. the §14 fabrication-never-rewarded regression + the `include_gain=False` case) + 8 platform `test_site_claim_index.py` (extraction/merge/freshness + single-flight coalescing) + 40 platform `test_ecommerce.py`. All green; platform ruff (CI ruleset) + `py_compile` clean. CI (nlp-api-tests + python-tests + lint-typecheck) green on the PR head.

**⚠️ LIVE VALIDATION OF THE GAIN PATH IS STILL PENDING (post-deploy, Railway shell — sandbox egress-blocked):** run an ecommerce score on a client that HAS a site (so `resolve_index_for_request` builds a real index) and confirm the `topic_vector.information_gain` block populates with `realized`/`ungrounded_novelty` verdicts, that an ungrounded on-vector claim scores 0 + is flagged, and that a reoptimize's coaching block lists real missing site facts. The P0 centering/coverage already validated live.

**NOT built — the next phases (need their own build + owner scope):** **P2** — the emotional-arc rubric dimension. Extending the whole measure (centering + coverage + gain) to the **Local SEO / service / blog scorers** is deferred (P1 is ecommerce-only). The §13 fixes (`never_use_terms` coverage-exclusion + title-in-scorer, empty-entity diagnosis) are separate task cards this PR doesn't touch. **The site claim index has NO scheduled refresh** — it's built on the first ecommerce score per client per 30-day TTL; a standalone "refresh the index" job/route is a possible follow-up.

---

## ⏩ Update — 2026-09-16 · **Content Gap Analyzer — v1.1 follow-ups (a)+(b) BUILT + MERGED (PR [#1166](https://github.com/kssabraw/ar-tools/pull/1166), squash `e4f3f2d`)**
...
**Built (surface = API + scheduler + frontend, all behind the flag):**
- **API** `routers/content_gap.py` + `models/content_gap.py` (registered in `main.py`; **never imports nlp**): `GET …/content-gap` (page bootstrap — `{enabled, auto_enabled, budget_remaining, runs}`; empty runs while dark, never 403 so the workspace card self-hides), `GET …/content-gap/estimate` (the **free** preflight — pure `estimate_max_calls`/`estimate_scan`: scope + worst-case call ceiling + remaining budget; **no spend, no enqueue**), `GET …/content-gap/runs` + `GET …/content-gap/runs/{id}` (list + detail; detail returns the nested `gap`/`onpage_diff`/`competitors` jsonb **verbatim as a plain dict** — no strict `response_model`, the ReoptAction strip lesson — and the frontend polls `run.status` here, so there's no separate job-status endpoint), `POST …/content-gap/scan` (**`require_staff`**, **503 while dark**, 429 over budget → `enqueue_content_gap_scan(trigger="manual")`, returns `{run_id}`), `GET …/content-gap/runs/{id}/export` (server-side CSV).
- **`services/content_gap.py` additions**: `budget_remaining()` (mirrors `domain_intel`), pure `estimate_max_calls()` (worst-case = keyword_count × `estimate_deep_calls(max_competitors)`) + `estimate_scan()`, `list_runs()`/`get_run_detail()` (detail sorts gaps-first, wins-last), pure `build_run_csv_rows()`/`CSV_HEADERS`, and `enqueue_due_content_gap_scans()`.
- **Scheduler** `enqueue_due_content_gap_scans` on the shared `gsc_scheduler` daily block (next to `domain_intel`) — eligible = clients with an active tracked keyword carrying a `canonical_url` (the money-page auto-scope), interval-gated via the `async_jobs` completed-history (not run rows, so a wins-only run still counts), self-gated on `content_gap_enabled` + `_auto_enabled` + DFS creds + budget, fully exception-guarded.
- **Frontend** `pages/ContentGap.tsx` + a "Content Gap" workspace card (SEO Strategist section, `<GitCompareArrows>`, gated on the module's `enabled` flag) + route `clients/:id/content-gap` (dependency-free) — run-now (staff, via `useAuth().isStaff`) + scope/budget preview, run-history chips, the verdict table (verdict chip · client pos · AIO ✓/✗/N-A · competitor count), a per-keyword drill-in with the per-dimension diff (competitor set / authority / traffic / entities + on-page score / on-page structure — subtopics, word count, missing elements, missing schema), **each dimension rendering an `unavailable` state** (§4.1), the internal↔softened **"Client summary"** toggle (owner decision #2 — the raw competitor-by-competitor diff is internal-only; the client view is an opportunity list), CSV export, and a **"Reoptimize this page"** button deep-linking to the Local SEO reoptimize tool (`?tab=reopt&url=…`, §11.1).
- **`tests/test_content_gap.py`** — 50 tests (42 + 8 new pure-helper cases: `estimate_max_calls` scaling/clamp/page-traffic-off, `build_run_csv_rows` full-row / win-with-no-gap-payload / empty, `CSV_HEADERS` stability). `ruff` E9/F63/F7/F82 clean; frontend `tsc -b` + `eslint` clean. CI green (platform-api tests + lint/typecheck + Netlify preview); nlp/pipeline test suites correctly untriggered (diff touches neither service).

**Two design calls (recorded, not blockers):** (1) the frontend **polls `run.status`** on `runs/{id}` (the user-facing entity) instead of the PRD §8 "job-status poll" — functionally equivalent, simpler; (2) **"Reoptimize this page" is a deep-link** to the Local SEO reoptimize tool (opens it + shows the URL to paste) rather than a URL-prefilled `ReoptimizePanel` mount — kept out of the large `LocalSeoContent.tsx` for v1.

**Deferred (v1.1, confirm scope first):** URL-prefill wiring into `LocalSeoContent`'s reopt tab (so the button lands the page in the box, not just the tab); a client-facing per-run PDF/report; threading gap-only findings (e.g. a specific missing subtopic) into the reopt as supplementary `writer_notes`-style guidance (PRD §11.1 v1.1 — NOT as `deficiencies`); and the real DataForSEO daily ceiling for the 500/day meter placeholder.

## ⏩ Update — 2026-09-16 · **Content Gap Analyzer — Phase 1 (the deep dimensions) BUILT + MERGED (PR [#1158](https://github.com/kssabraw/ar-tools/pull/1158))**

Phase 1 of the **Content Gap Analyzer** (authority: `docs/modules/content-gap-analyzer-prd-v1_0.md` — read its top "Revision 2026-09-15" note + §3 (gap dimensions, tagged reuse/free/+1call/build) + §4/§4.1 (the on-page diff + its per-dimension empty/failure states) + §5 (reuse/build table) + §11.1 (the CORRECTED reopt handoff)). Builds on the merged Phase 0 foundations (#1143). **Still ships dark: `content_gap_enabled` default False; nothing auto-enqueues, nothing spends until it's flipped on and a scan is explicitly requested. NO migration** (reuses the `content_gap_scan` job; the `gap`/`onpage_diff` jsonb columns already exist from Phase 0). **DFS daily ceiling confirmed at 500/day (owner) — a placeholder to revisit once real usage is observed.**

**Built (fills the `gap` + `onpage_diff` jsonb the Phase 0 job left null; for each GAPPED keyword only — wins short-circuit, PRD §8, so a healthy client stays cheap):**
- **`services/content_gap.py` deep pass** (`_deep_dimensions`, best-effort per dimension — a failure is recorded in `gap.dimensions_unavailable` and the rest still assemble, PRD §4.1):
  - **Authority** (free, no new calls): `assemble_authority` + `build_authority_gap` — page + domain referring-domains, url-rating, and DR from the snapshot's already-captured `serp_snapshot_results` (page RD/UR) + `serp_snapshot_domains` (domain RD/DR) rows, with competitor medians, client deltas (positive = competitors ahead), and the standing "×10 DataForSEO tool-read, not Moz DA" caveat (§3.1 #2/#3).
  - **AIO citation gap** (free): `aio_gap_sources` — who's cited in the AI Overview that the client isn't, only when an AIO is present (§3.2).
  - **SERP + client entities**: one nlp `/analyze` (SERP-aggregated `google_entities`, the right shape for a gap) reused as `/score-page`'s `serp_analysis`; `/score-page` on the client URL supplies the client-side entity/structure/signal deficiencies filtered to the `entity_establishment`/`serp_signal_coverage` engines (`build_entity_gap` — there's NO endpoint for a raw client entity list, PRD §3.1 #1). **platform-api → nlp over HTTP; nlp is never imported** — a job-friendly best-effort `_post_nlp` (returns None on failure, never raises, unlike `local_seo_service._post_nlp`).
  - **Site traffic**: one batched `dataforseo_labs.fetch_bulk_traffic` over the client + competitor domains (`build_site_traffic_gap`, §3.1 #5).
  - **Page traffic**: a **LABELLED modeled estimate** `Σ(search_volume × forecasting.ctr_for_position(position))` over each domain's `fetch_ranked_keywords` rows filtered to the ranking URL (`estimate_page_traffic`/`build_page_traffic_gap`; `fetch_ranked_keywords` is domain-scoped and returns volume, not ETV — PRD §3.1 #6). Config-gated (`content_gap_page_traffic_enabled`) — the priciest sub-dimension (one ranked-keywords call per competitor + one for the client). A no-URL-match returns None (no signal), NOT zero.
  - **On-page diff**: `_scrape_signals` (`website_scraper.scrapeowl_fetch` → the Phase-0 `page_signals_from_html`) over the capped competitor set + the client URL → the Phase-0 `build_onpage_diff`. A failed/empty/bot-blocked client scrape marks the column `unavailable` per §4.1 — it isn't "client has zero of everything".
- **Budget = fail-CLOSED** (`_reserve` → the Phase-0 `reserve_content_gap_calls` RPC): gates ALL paid calls BEFORE spending, and **deliberately stricter than `domain_intel`'s fail-OPEN `reserve_budget`** — an accounting error (RPC exception) BLOCKS the spend rather than proceeding, so no paid call ever runs without a confirmed reservation (PRD §7). Each gapped keyword reserves `estimate_deep_calls(competitor_count, page_traffic_enabled)` up front (an upper bound, errs high); a refused reservation keeps the shallow verdict row and marks the keyword `budget_limited` (a re-run recovers it once budget frees). `cap<=0` disables the guard.
- **Fresh-capture path** (replaces the Phase 0 `no_snapshot` skip): a keyword with no reusable snapshot now **enqueues a per-keyword `serp_snapshot` capture** (budget-reserved at `content_gap_snapshot_call_estimate`≈22 — a full capture is ~20-25 DFS calls — and count-bounded by `content_gap_max_fresh_captures`=10/run, a second guard against a fresh-client fan-out) and **DEFERS to the next run**: a freshly-enqueued job hasn't run yet, and the scan never blocks on another job, so "re-read" lands on the next scan. `resolve_scope` now returns `keyword_id` to drive it. The run settles **`partial`** when any keyword was deferred (pending capture / budget-limited / unmeasured), else `complete`; the job result carries `deep_analyzed`/`pending_capture`/`budget_limited`/`no_snapshot`.
- **New config** (`config.py`): `content_gap_page_traffic_enabled` (True — kill-switch for the modeled page-traffic + its ranked-keywords calls), `content_gap_snapshot_call_estimate` (22), `content_gap_max_fresh_captures` (10).
- **`tests/test_content_gap.py`** — 42 tests (24 Phase 0 + 18 new): `estimate_deep_calls`, `_norm_url`, `estimate_page_traffic` (URL-match / no-match→None / fractional CTR / volumeless-row-skip), `build_authority_gap` + `assemble_authority` (medians/deltas, client-not-ranking uses the domain row, url-normalized competitor match), `build_site_traffic_gap`, `build_page_traffic_gap` (labelled modeled), `build_entity_gap` (SERP mapping + deficiency filter by `engine_key`/`engine`), `_reserve` (confirmed / over-cap / cap-0-disables / **fail-closed on exception**), and the full `_deep_dimensions` pass with ALL DFS/nlp/scrape calls mocked (assembles every dimension; degrades cleanly when nlp fails AND there's no client URL). `ruff check` clean (E9/F63/F7/F82). All external calls mocked; **live paths (DataForSEO / nlp / ScrapeOwl) are egress-blocked in-sandbox and stay worker-verified post-deploy.**

**Deferred to later phases (each needs its own owner scope confirmation — DO NOT build without asking):** `routers/content_gap.py` + `models/content_gap.py` (the HTTP surface: `POST …/content-gap/scan`, `GET …/content-gap/runs[/{id}]`, the free `GET …/content-gap/estimate` preflight, job-status poll, CSV export); the monthly `enqueue_due_content_gap_scans` scheduler hook on the shared `gsc_scheduler` (daily due-check, `content_gap_interval_days`=30, gated on `content_gap_auto_enabled` + budget); and `pages/ContentGap.tsx` + the "Content Gap" workspace card — the internal diff + the softened client-facing summary (owner decision #2) + the verdict table + per-keyword drill-in + the "Reoptimize this page" button routing through the existing `page → /score-page → /reoptimize-page` flow (PRD §11.1 — the gap report is presentation-only; the pipeline takes `/score-page`'s 8-engine deficiency dicts, NOT a free-form gap payload).

## ⏩ Update — 2026-09-16 · **Topic-Vector Centering + Information Gain — P0 (report-only) — BUILT + MERGED (PR [#1156](https://github.com/kssabraw/ar-tools/pull/1156))**
...
**NOT built — the next phases (need their own build):** **P1** — the per-client structured-fact **site claim index** + its platform→nlp payload contract + the *scored* Information Gain dimension coached into the reopt loop (the load-bearing anti-fabrication grounding) — **now BUILT (PR #1164 — see the top entry).** **P2** — the emotional-arc rubric dimension. Extending the measure to the Local SEO / service / blog scorers is also deferred. Related §13 fixes (`never_use_terms` coverage-exclusion + title-in-scorer, empty-entity diagnosis) are separate task cards P0 doesn't touch.
...
**The reuse decision (the one the PRD left to the builder).** PRD §10 says "reuse the Website-Builder theme render." That path (`website_theme` → `tokens.css` → a per-site GitHub repo → Cloudflare deploy → a live Astro site) is heavyweight, async, and **makes an LLM call** (role-naming) — it produces a live URL that can't embed in a PDF fragment and would break the render's core invariant (**the LLM is never called at render time**, §5.2). So Phase 5 reuses the **token → page CONCEPT** inside the existing WeasyPrint pipeline instead: the guide already owns roled swatches + fonts + a type scale + tagline + worked voice examples + a logo, so the mockups are **pure, deterministic HTML/CSS assembly** over that stored record. No new infra.

**Backend — `services/brand_guide_mockups.py` (NEW, pure, unit-tested `tests/test_brand_guide_mockups.py`):**
- `resolve_palette(guide)` → a mockup token map (`primary`/`accent`/`secondary` from the LLM-assigned swatch roles, skipping `not_brand`; `on_primary` picked by WCAG contrast reusing `brand_guide_coherence`; `ink`/`paper` from the darkest/lightest measured colour with slate/white fallbacks; `heading_font`/`body_font` from the census). Degrades to the `visual_census` colours when there are no synthesized swatches; returns `{}` (→ the section is omitted) when there's no usable palette.
- Five mockup builders + `build_applications_section(guide, ctx, *, logo_src)` — fully inline-styled so the section is testable in isolation. **No invented copy**: headlines/taglines/CTAs come from the synthesized `voice_examples`/`tagline`/`boilerplate`; a missing CTA falls back to the generic UI word "Learn more" (a button label, never a brand/product claim, §5.3); contact lines use only the client's real name/website/phone/address; a missing logo → a text wordmark (no logo synthesis, §13). Identical in both render profiles (§4.7 — the only internal↔client delta stays the coherence/audit treatment).
- `brand_guide_render.build_guide_html` drops the section in after Imagery; `gather_render_context` now also pulls real GBP `phone`/`address` for the card/letterhead.

**Frontend — `frontend/src/pages/BrandGuide.tsx`:** a read-only **Applications** preview listing the five mockups in the PDF + a palette hint, gated on the same "has a palette" condition as the backend. It deliberately does **not** re-render the mockups in React — a second rendering engine would drift from the PDF; the mockups are the PDF deliverable, reached via the existing Client PDF / Internal audit download buttons.

**Tests / verification:** `tests/test_brand_guide_mockups.py` (palette resolution — roles / census fallback / not_brand skip / empty / ink+paper defaults; all five mockups + tokens present; the degrade omission; CTA + wordmark + contact fallbacks; escaping; profile-independence; the no-LLM source guarantee) + a render-layer integration assertion (section present with a palette, omitted without) + a `gather_render_context` NAP test. **204 brand_guide backend tests pass; the full document renders through WeasyPrint in BOTH profiles** (client + internal) with logos + NAP. Frontend `tsc -b` + `eslint` clean. Live capture/render/Drive stays worker-verified post-deploy (egress-blocked in-sandbox, same as the rest of the module); nothing auto-enqueues, so nothing spends until a guide is requested.

## ⏩ Update — 2026-09-16 · **Content Gap Analyzer — Phase 0 (foundations) BUILT + MERGED (PR [#1143](https://github.com/kssabraw/ar-tools/pull/1143))**

First code for the **Content Gap Analyzer** module (authority: `docs/modules/content-gap-analyzer-prd-v1_0.md`, PRD revised 2026-09-15 after an adversarial review — read its top "Revision" note + §5 reuse/build split + §11 open items first). The module answers, per client, for selected **money-pages × main keywords**: *are we winning the SERP (top-10 organic AND cited in the AI Overview, when an AIO exists), and if not, exactly what do the competitors above us have that we don't?* It is an **assembly / diff / presentation layer** over surfaces the suite already runs (`serp_snapshots`, nlp `/analyze` + `/score-page`, `dataforseo_labs`, `page_structure_eval`) — **no new scoring engine**, and it does NOT replace the reoptimize pipeline (it diagnoses; the existing `page → /score-page → /reoptimize-page` flow fixes, PRD §11.1). **Ships dark: `content_gap_enabled` default False; nothing auto-enqueues, nothing spends until it's flipped on and a scan is explicitly requested.**

**Four owner decisions (locked 2026-09-15/16, were PRD §11.2 open):**
1. **DFS daily paid-call ceiling = 500/day** (the fail-closed meter default; `content_gap_daily_call_budget`).
2. **On-page diff = internal-only + a softened client-facing summary** (matches the suite's client-report tone ruling — no scare copy). *Not yet surfaced (no UI in Phase 0); a Phase-1+ frontend concern.*
3. **Monthly auto-scope = money pages from `tracked_keywords.canonical_url`** (`resolve_scope` derives it; no new inventory table).
4. **Snapshot reuse window = 30 days** (`content_gap_snapshot_max_age_days`; reuse the rank tracker's existing weekly capture, only fetch fresh — ~20–25 DFS calls/keyword — when stale/absent).

**Built + merged (CI green — platform-api tests + lint/typecheck + Netlify; head `89b35d4`):**
- **Migration `20260916180000_content_gap.sql` — APPLIED LIVE (verified: 3 tables + RPC + CHECK contains `content_gap_scan`):** `content_gap_runs` (one per client × trigger × created_at, `trigger ∈ {scheduled,manual}`, status, rollups keywords_analyzed/wins/gaps) + `content_gap_keywords` (one per run × keyword × page — client_position, `aio_present`/`in_aio`, `verdict ∈ {win,aio_gap,organic_gap,full_gap}`, `competitors`/`gap`/`onpage_diff` jsonb, `serp_snapshot_id`/`captured_fresh` provenance) + `content_gap_usage` + the **fail-closed** `reserve_content_gap_calls(day,n,cap)` RPC (copied byte-for-byte from `reserve_domain_intel_calls`). `async_jobs` CHECK **rebuilt from the LIVE constraint** (per the standing rule; dated `…180000` so a from-scratch rebuild ends with the complete set) + `content_gap_scan`. All RLS-on, service-role only.
- **`config.py`** — `content_gap_{enabled=False, auto_enabled=False, daily_call_budget=500, snapshot_max_age_days=30, interval_days=30, max_competitors=5}`.
- **`services/content_gap.py`** — the **pure core** (unit-tested, no I/O): `compute_verdict` (the §2.1 **three-valued** win/gap matrix — the AIO axis only yields a gap when an AIO is actually present on the SERP, so a no-AIO keyword the client ranks well for is never a permanent "AIO gap" with an empty competitor set), `client_cited_in_aio`/`aio_gap_sources`, `resolve_competitors` (the §6 "both" union — organic-above ∪ registered-`client_competitors`-in-SERP, deduped by domain, strongest-first by position, capped at `max_competitors`, the client excluded by domain AND the snapshot's `is_client` flag), `page_signals_from_html` (title/meta/headings/word-count/block-types/JSON-LD `@type`/element flags via `page_structure_eval`), and the **`build_onpage_diff(client_signals, competitor_signals)`** assembler (subtopic-heading gap, word-count-vs-median, structural-element gap, schema gap, title/meta — each dimension degrading independently, and an unavailable client column marked, not counted as "has zero of everything", PRD §4.1). Plus the read/scope helpers: `latest_reusable_snapshot` (the cost-critical §8 reuse — newest complete/partial snapshot within the window), `_snapshot_result_rows`, `resolve_scope`, `_registry_domains`. **The `content_gap_scan` job** (`enqueue_content_gap_scan` + `run_content_gap_scan_job`): resolves scope → reads the latest reusable snapshot per keyword → computes verdict + (for gaps) the competitor set from the snapshot's result rows → persists one `content_gap_keywords` row each → settles run rollups + job result. **Wired into `job_worker` dispatch** (lazy import). NOT freeze-gated (observational — correct).
- **`tests/test_content_gap.py`** — 24 pure-core unit tests (verdict matrix incl. every §2.1 branch, AIO helpers, competitor union/dedup/cap/is_client-exclusion, page-signal extraction, the on-page diff incl. client-unavailable + no-competitors, snapshot freshness). All green; `ruff check` clean (repo-wide, CI rule set E9/F63/F7/F82); mypy N/A (file not in the typed allowlist).

**Adversarial self-review (done before merge — the fixes are in `89b35d4`):**
- **Wrapped the job body in try/except → settle `failed`.** The worker loop only LOGS an unhandled handler exception (`job_worker.py:1302`), it does not settle. A transient DB error after `status='running'` would leave the run+job stuck `running` until the reaper requeues and RE-RUNS the whole scan, double-inserting `content_gap_keywords` (no idempotency guard). Settling failed makes a re-run impossible → no duplicates; on-demand re-run is the recovery path.
- **Stopped fabricating an `organic_gap` verdict for keywords with no fresh snapshot.** Phase 0 reads EXISTING snapshots only; an unmeasured keyword is UNKNOWN, not a gap (the verdict enum can't express that), and counting it inflated the run's gap rollup. It's now skipped and counted separately as `no_snapshot` in the job result; `analyzed`/`gaps` reflect only measured keywords. The fresh-capture phase will handle them.
- **`resolve_competitors` also excludes the client's own row by the snapshot's `is_client` flag** (a client with no `website_url` yields an empty `client_domain`, so domain matching alone can't identify it). +1 unit test.
- **Two "obvious bugs" verified NOT bugs before reporting:** postgrest-py 2.31.0 `_unique_columns` sends the UNION of keys across a bulk insert and `default_to_null=True` fills missing ones as NULL, so a heterogeneous insert is fine; `select 'now()'::timestamptz` casts correctly (the repo-wide settle idiom works).

**Deferred to later phases (nothing below is built):**
- **Phase 1 — the deep dimensions:** for each gapped keyword, scrape the capped competitor set (`website_scraper.scrapeowl_fetch`) + `page_structure_eval` → `page_signals_from_html`, call nlp `/score-page` on the client URL (entity/structure/signal deficiencies — the client side of the entity diff; there is NO endpoint for a raw client entity list) and one nlp `/analyze` (SERP-aggregated entities), pull `dataforseo_labs.fetch_bulk_traffic`/`competitors_domain` (site traffic) + the **modeled** page-traffic estimate `Σ(volume × forecasting.CTR_CURVE[pos])` (labelled "estimated (modeled)" — `fetch_ranked_keywords` is domain-scoped and returns volume not ETV, PRD §3.1 #6), then call `build_onpage_diff` end-to-end and write `gap`/`onpage_diff` on the row. The fail-closed meter must gate this (reserve calls before spending); wins short-circuit (no deep dimensions).
- **Fresh-capture path:** when `latest_reusable_snapshot` is None/stale, enqueue a `serp_snapshot` capture and re-read (instead of skipping as `no_snapshot`).
- **Surface:** `routers/content_gap.py` (`POST …/content-gap/scan`, `GET …/content-gap/runs[/{id}]`, free `GET …/content-gap/estimate` preflight, job-status poll, CSV export) + `models/content_gap.py`.
- **Cadence:** the monthly `enqueue_due_content_gap_scans` hook on the shared `gsc_scheduler` (daily due-check, `content_gap_interval_days`=30, gated on `content_gap_auto_enabled` + budget).
- **Frontend:** `pages/ContentGap.tsx` + a "Content Gap" workspace card (route `clients/:id/content-gap`) — the internal diff + the softened client summary (owner decision #2), the verdict table, per-keyword drill-in, and a "Reoptimize this page" button routing through the existing `/score-page → /reoptimize-page` flow (PRD §11.1 — the gap report is presentation only; the pipeline takes `/score-page`'s 8-engine deficiency dicts, NOT a free-form gap payload).

**Open item still unanswered (PRD §11.2):** the real DataForSEO daily ceiling is a 500/day placeholder — confirm against the account's true limit before Phase 1 starts spending at scale.

## ⏩ Update — 2026-09-16 · **Brand Guide Generator — Phase 4 (frontend + the enforcement-suggestion surface + the regulated sign-off approval UI) — BUILT + MERGED (PR [#1153](https://github.com/kssabraw/ar-tools/pull/1153), squash `881122e1`)**

Phase 4 of the Brand Guide Generator (PRD `docs/modules/brand-guide-generator-prd-v1_0.md` §9 Frontend / §4.7 render profiles / §4.8 suggest-only enforcement / §6 the `edited` flag + the awaiting_signoff→`brand_guide_render` approval seam / §5.3b the sign-off approver / §12 Q1 logo adopt + Q3 proposed-palette / §10 Phase 4). **This is the FIRST HTTP surface the module has** — Phases 0–3 shipped the pipeline (capture → extract → vibe → synth → render + the two async jobs) with NO routes and NO UI. **Phase 4 COMPLETES the v1 module** (only the deferred Phase 5 applications/mockups remains). **No migration** — every column it uses (`edited`, `renders`, the `awaiting_signoff` status) already exists. Still gated on `brand_guide_enabled` (default False in code; **TRUE on PLATFORM**); the UI renders a dark state when off, and nothing auto-enqueues.

**Backend — `writer/platform-api/routers/brand_guide.py` (NEW, registered in `main.py`) + `models/brand_guide.py` (NEW):**
- `GET  …/brand-guide/status` → `{enabled}` (the page's dark-state gate).
- `POST …/brand-guide/generate` (`require_staff`, 503 while dark) → `enqueue_brand_guide_generate` (source URL defaults to `clients.website_url`; optional `pages` scope override; threads `auth.user_id` for the Activity ping). Returns the queued row.
- `GET  …/brand-guide` (list history, client-scoped) + `GET …/brand-guide/{id}` (the full row; **re-signs** `pdf_url` + every `renders[*].pdf_url` on read so a stale link never 404s).
- `POST …/brand-guide/{id}/render-approve` (`require_staff`; 409 unless `status == awaiting_signoff`) → `enqueue_brand_guide_render`, then flips the row to `rendering` so the UI moves without a poll gap (§5.3b).
- `GET  …/brand-guide/{id}/download?profile=internal|client` (`brand_guide_edit.resolve_render_path` → `client_report._signed_url`; 409 `not_rendered` / 422 `invalid_profile`) — the two render profiles are a **download param** over the one stored record (§4.7).
- `PUT  …/brand-guide/{id}` (`require_staff`) — the **structured-field editor** (§9): `set_field` (tagline/positioning/mission), `set_voice_example`, `swatch_rename`/`_drop`/`_flag_not_brand`; validated + applied by the pure `brand_guide_edit`; sets `edited=true` (nothing set it before); a no-op edit → 409 `no_change`. Only the **Proposed** layer (`synthesized`) is mutated — never the measured `visual_census`.
- `POST …/brand-guide/{id}/logo-adopt` (`require_staff`, §12 Q1) — adopt a `visual_census.logo_candidate` (guarded: only a URL the census actually surfaced) → re-host into the public `client-logos` bucket → set `clients.logo_url`; **NEVER silently overwrites** an existing logo (409 `logo_exists` unless `replace:true`).
- `GET  …/brand-guide/{id}/voice-suggestions` (§4.8) → `{text, editor_path, has_suggestions}` — the copyable refined voice/messaging block + the deep-link to the EXISTING Brand Voice editor. **Suggest-only, no write path** (the two facts forcing this: the Voice Card is derived cache; writing `brand_voice` flips `source→"user"` and blocks the auto-scan — the operator must choose that in the editor).
- **Permissions:** every mutation is `require_staff`. The suite has **no per-client owner column**, so §5.3b's "admin OR the client's owning staff member" approver collapses to the staff tier — matching the sibling GBP Profile Editor's `apply_edit` gate.

**Pure core — `services/brand_guide_edit.py` (NEW, unit-tested `tests/test_brand_guide_edit.py`, 12 cases):** `validate_ops`/`apply_edits` (deep-copy, never mutates input; a swatch op with no match is a counted no-op so the router can 409; a `set_*` with an empty value CLEARS the field; a swatch drop/flag recomputes the deterministic 60/30/10 usage map), `pick_logo_candidate` (census-URL guard), `resolve_render_path` (per-profile → top-level client mirror fallback), `build_voice_suggestion_text` (the §4.8 copyable block).

**Frontend — `frontend/src/pages/BrandGuide.tsx` (NEW) + route `clients/:id/brand-guide` (App.tsx) + a "Brand Guide" workspace card (`ClientWorkspace.tsx`, Reporting section, `Palette` icon):** mirrors Client Reports — generate on-demand (+ an "Options" source-URL override), history table with **live status polling** (polls while any guide is queued/capturing/synthesizing/rendering), a per-profile **download** (Client PDF / Internal audit, re-signed on click), the Documented/Proposed **section preview** (measured palette + named swatches, aesthetic descriptors + coherence flags), the **structured-field editor** (tagline/positioning/mission + voice examples batched into one PUT; swatch rename/drop/flag-not-brand as discrete actions), the **logo adopt** grid (adopt / replace-anyway on conflict), the **awaiting_signoff** admin/staff "Approve & render" action (`isStaff`-gated), and the **suggest-only** voice surface (copy-to-clipboard + "Open Brand Voice editor" link). `BrandGuide` type added to `lib/types.ts`. `tsc -b` + `eslint` clean.

**Adversarial-review fixes folded into the PR before merge (commits `db845d8e`→`a368e480`):** (1) `swatch_flag_not_brand` now recomputes the 60/30/10 `usage_ratios` map (it only did on `swatch_drop` before — a flag alone left the flagged colour in the render's usage block; the prior test masked it by dropping in the same call, now covered by `test_flag_alone_recomputes_usage_map`); (2) regenerating while the latest guide carries operator edits now `window.confirm`s first (§6 "warns + offers a new version" — generation always creates a NEW version and never overwrites, so it's a warning, not a guard). Known-and-accepted (self-review, low): `PUT` edit isn't status-gated (UI-gated; a mid-capture edit would be clobbered by finalize, benign); double render-approve enqueues two idempotent render jobs; concurrent-edit is TOCTOU last-write-wins but the delta design (each PUT re-reads current `synthesized`) makes sequential edits compose. Standalone strict `mypy` flags 8 `isinstance-else-{}` false positives in `brand_guide_edit.py` — NOT CI-enforced (CI mypy checks only `rank_materialize.py`), all runtime-guarded.

**Verification (all green at merge on `a368e480`):** CI ruff ✅ / mypy ✅ / pytest ✅ / Netlify ✅; pure `brand_guide_edit` unit-tested; the router registers 9 routes. **Live full path (capture/render/Drive) stays worker-verified post-deploy** — egress-blocked in-sandbox (ScrapeOwl/DataForSEO/Apps Script), unchanged from Phases 1–3.

**The Brand Guide Generator v1 module is COMPLETE (Phases 0–4 all merged).** Only the **deferred Phase 5** (applications/mockups — reuse the Website-Builder theme render) remains, and needs its own owner greenlight (PRD §10 Phase 5 / §3 section 9).

## ⏩ Update — 2026-09-15 · **Brand Guide Generator — Phase 3 (PDF render + render profiles + the `brand_guide_render` job) — BUILT + MERGED (PR [#1150](https://github.com/kssabraw/ar-tools/pull/1150))**

Phase 3 of the Brand Guide Generator (PRD `docs/modules/brand-guide-generator-prd-v1_0.md` §4.6 assemble+store / §4.7 Render + the internal|client render profiles / §6 data model + the awaiting_signoff→brand_guide_render approval seam / §10 Phase 3 / §11 acceptance #6). Renders the assembled `brand_guides` record to a **portable PDF** in the `reports` bucket + delivers it to the client's Drive folder. Builds on merged Phase 2 (#1145). **NO frontend, NO enforcement-suggestion surface, NO sign-off approval UI** (Phase 4). Still gated on `brand_guide_enabled` (default False in code; **TRUE on PLATFORM**); nothing auto-enqueues, so nothing spends until a guide is explicitly requested. **Post-merge CI fix:** the `test_no_reserved_logrecord_keys_in_extra` guard rejects `name` as a logging `extra` key (a reserved `LogRecord` attribute); the Drive-failure warning in `_deliver_client_pdf` now uses `client_name` (folded into the same PR before merge).

**Built (deterministic assembly over the stored record — the LLM is NEVER called at render time, §5.2; best-effort / degrade-never-fail throughout, §5.4):**
- **`services/brand_guide_render.py`** (pure HTML/section builders unit-tested `tests/test_brand_guide_render.py` 27; WeasyPrint mocked/lazy in tests, exercised for real in-sandbox — client_report already proves WeasyPrint renders here). A self-contained `<!doctype html>` + inline print `_CSS` (A4/@page, cover, dual-layer **Documented + Proposed** sections per §3, swatch grid with Hex/RGB/CMYK/HSL + share, WCAG pairing rows, mood-axis bars, type specimens, the coherence "where the brand is leaking" block, logo + imagery/voice-example blocks). The homepage screenshot (read back from the `brand-guides` bucket) + logo are inlined as base64 data URIs so the PDF is portable. Sections degrade to a muted "not captured" line (never a blank), so a client with no site still renders a full guide from its voice/ICP assets (AC #7).
- **Render profiles (§4.7):** ONE `profile: internal | client` render-time param over the one stored record — the ONLY delta is the coherence/audit treatment. **internal** = the blunt `Coherence audit` (the deterministic flags verbatim: title + detail). **client** = the identical findings reframed as forward-looking **"Opportunities to sharpen"** (the LLM already reframed them into `coherence.narrative` + `section_gaps` at synthesis time — no LLM at render), plus the white-label footer (`client_report_agency_name`). BOTH ship; `renders` (new jsonb, migration `20260916180000`, **applied live**) holds `{internal:{storage_path,pdf_url,rendered_at}, client:{…,delivery}}`, and the top-level `storage_path`/`pdf_url` mirror the CLIENT profile (the deliverable).
- **Trigger (§6):** a **non-regulated** guide (status resolved to `done`) renders **inline** in `brand_guide_generate` right after synthesis (`_finalize_guide` → `render_and_store_guide`: status → `rendering` → `done`, `renders`/`pdf_url` set, gated on `brand_guide_enabled` AND `brand_guide_render_enabled`). A **regulated** client (`content_compliance_mode != 'off'`) stops at `awaiting_signoff` and is rendered LATER by the separate **`brand_guide_render`** job (`run_brand_guide_render_job`, wired in `job_worker` — replaces the honest "not implemented" settle; registered in `SINGLE_JOB_REGISTRY` so it pings the initiator) enqueued on a human approval (`enqueue_brand_guide_render`). The approve endpoint/UI is Phase 4 — this builds the job + render + store + deliver so Phase 4 just enqueues it. The GBP-Profile-Editor "status + a 2nd job" pattern (jobs can't pause mid-run).
- **Store + deliver (reuse §7, no second renderer/Drive path):** `client_report.render_pdf` (WeasyPrint, lazy-imported), `_store_pdf`/`_signed_url` (the `reports` bucket), and Drive via `google_docs.resolve_drive_folder(client,"brand_guide")` + `upload_pdf` — the **client** profile is the one delivered to Drive. NOT freeze-gated (observation/deliverable, §5.5).
- **A render failure records `status='error'` + an honest note on the row** and never crashes the worker — the census/vibe/synthesized data already persisted survives, so the guide is simply flagged for a re-render (nothing lost). A Drive-delivery failure is recorded (`renders.client.delivery`), not raised.
- **Config:** `brand_guide_render_enabled` (default True — skip guard on top of `brand_guide_enabled`; turn render off while capture/synthesis are tuned).
- **Tests:** +27 in `test_brand_guide_render.py` (pure section builders per stored shape, the profile delta internal↔client, "not captured" degrade paths, render_and_store both-profiles/mirror/Drive/failure paths, the render job flag/missing-id/complete paths, enqueue) + 2 wiring tests in `test_brand_guide.py` (inline render fires for a `done` guide and folds render status/pdf_url into the result; render does NOT fire for a regulated `awaiting_signoff` guide). All 211 brand-guide + activity + freeze tests green locally; both profiles + the empty/no-site case verified to render to a valid PDF in-sandbox.

**Worker verification (PENDING — post-deploy):** Drive delivery + the live full path are egress-blocked in-sandbox (ScrapeOwl/DataForSEO for capture; Apps Script for Drive). With `brand_guide_enabled` TRUE, enqueue one `brand_guide_generate` job for a non-regulated client with a website — the newest `brand_guides` row should reach `status='done'` with `renders->'client'->>'pdf_url'` set, a PDF in the `reports` bucket at `<client_id>/brand-guide/<guide_id>-client.pdf`, and (if a Drive folder is configured) `renders->'client'->'delivery'->>'drive' = 'ok'`. For a regulated client (`content_compliance_mode != 'off'`) the generate job stops at `awaiting_signoff`; enqueue a `brand_guide_render` job for its guide id to render + deliver.

**STOPPED before Phase 4** (the frontend workspace card/page, the two render-profile download toggle, the structured-field editor, the **enforcement-suggestion surface**, and the **regulated sign-off approval UI** that enqueues `brand_guide_render`) per the task — each needs its own owner go.

## ⏩ Update — 2026-09-15 · **Brand Guide Generator — Phase 2 (grounded synthesis + coherence check + regulated guardrail) — BUILT + MERGED (PR [#1145](https://github.com/kssabraw/ar-tools/pull/1145))**

Phase 2 of the Brand Guide Generator (PRD `docs/modules/brand-guide-generator-prd-v1_0.md` §4.4 pull assets / §4.5 synthesize / §4.3 coherence check / §3 section table / §5.3 guardrail / §10 Phase 2). Adds the **Proposed** layer on top of the merged Phase 1 capture (#1141) + Phase 1.5 vibe read (#1144). **NO PDF render, NO frontend** (Phase 3/4). Still gated on `brand_guide_enabled` (default False in code; **TRUE on PLATFORM**); nothing auto-enqueues, so nothing spends until a guide is explicitly requested. **No migration** — the `synthesized` / `awaiting_signoff` columns already exist (migration `20260916170000`).

**Adversarial self-review hardening (folded in before merge):** `brand_guide_synthesis._slist` now wraps a scalar string as one item rather than splitting it into characters (a `report_llm` cross-provider fallback can return a string where the schema declares an array — `tagline_options`/`key_messages`/imagery `dos`·`donts`); `brand_guide._finalize_guide` wraps `run_synthesis_for_guide` so an unexpected synthesis error (a malformed asset, a helper raising) degrades to a **`done` guide with the census + vibe intact**, never an `error`-status guide (§5.4 — only the two LLM calls were wrapped before); the "both LLM halves failed but the deterministic layer survived" note reads "synthesis deterministic-only (both LLM calls failed)" instead of "naming only"; and the dead `libinal` token was removed from the guardrail health-outcome vocabulary. +3 tests (scalar-string, deterministic-only note, synthesis-exception-still-finalizes-`done`).

**Built (best-effort / degrade-never-fail throughout, PRD §5.4):**
- **`services/brand_guide_coherence.py`** (pure, unit-tested `tests/test_brand_guide_coherence.py` 21) — the two deterministic pieces the LLM narrates but **never computes** (§5.2): (a) **WCAG contrast pairings** (`wcag_contrast`/`contrast_level`/`contrast_pairings`) — the accessible text-on-swatch rows for the Color section, computed in Python; note the optimal-of-white/black text is mathematically guaranteed to clear AA body (crossover minimum ~4.58:1), so `passes_body` is an invariant. (b) **The coherence check** (`build_coherence_flags`) — the audit payoff: because the measured census and the felt vibe read are produced INDEPENDENTLY, it cross-checks them and flags divergence — `neutral_sprawl` (≥4 low-sat grays), `palette_sprawl` (≥8 colours), `type_scale_sprawl` (≥9 sizes), `font_sprawl` (>2 in-use faces), and the headline **`vibe_execution_gap`** ("reads premium, but 5 near-duplicate neutrals → consolidate"), which fires only when a sprawl CONTRADICTS an explicit premium/minimal vibe axis.
- **`services/brand_guide_guardrail.py`** (pure, unit-tested `tests/test_brand_guide_guardrail.py` 12) — the regulated §5.3(a) **input-filter, not just an output-scan**: for a client with `content_compliance_mode != 'off'` it splits the synthesis corpus into sentences and **excises any claim-shape sentence BEFORE synthesis** (closing the laundering hole where the client's own site copy carries a gray-area claim). Two detectors: it **reuses `content_compliance.scan_text`** critical rules (dosing/branded-equivalence/guaranteed-results/advocacy) AND adds the broader efficacy-verb + health-outcome net the PRD names (`supports muscle recovery`, `clinically proven`, …). A non-regulated / unknown mode is a **no-op** (the universal prompt hygiene carries the weight there).
- **`services/brand_guide_synthesis.py`** (pure helpers unit-tested `tests/test_brand_guide_synthesis.py` 17; LLM mocked) — the orchestration: **`pull_assets`** reads the client's owned assets via `resolve_brand_guide_text` / `resolve_icp_text` (§4.4 — NOT the legacy `*_text` columns), the cached Voice & Audience Card (`clients.voice_card.card`, never re-distilled), `differentiators`, `logo_url`/`gbp.logo`. **Two bounded forced-tool calls** via `report_llm.run_forced_tool` (§12 Q4), each independently best-effort: **naming** (Haiku, `brand_guide_naming_model`) NAMES + ROLES the measured tokens (swatches anchored to the EXACT census hexes — an invented hex is dropped, never used — + a named type scale + imagery/iconography + tagline options); **messaging** (Sonnet, `brand_guide_synthesis_model`) writes positioning + mission + tagline + worked voice examples + we-say/we-don't + key messages + boilerplate + the coherence narrative + a per-section gap analysis, over the **guardrail-filtered** corpus. The deterministic coherence flags, WCAG pairings, and the 60/30/10 usage mapping (`usage_ratios_from_roles`) are attached in code (§5.2). **Universal guardrail** (every client): a prompt exclusion block — never a product claim / efficacy / dosage / safety / "FDA" / any ungrounded fact.
- **Wired into the pipeline** (`services/brand_guide.py`) — a shared `_finalize_guide` now runs vibe (1.5) → synthesis (2) → the row write; **both** the captured path and the no-site path route through it, so a client with no website still gets the synthesized Proposed layer from its voice/ICP assets (§5.4 / AC #7). Status finalizes `done`, **except a regulated client whose synthesis produced content finalizes `awaiting_signoff`** (§5.3b — the render/approval flow stays Phase 3/4; nothing to approve against yet). A disabled flag / no LLM output / a failed half all degrade (partial or omit `synthesized`) and the guide still finalizes.
- **Config:** `brand_guide_synthesis_enabled` (default True — skip guard on top of `brand_guide_enabled`), `brand_guide_naming_model` (`claude-haiku-4-5-20251001`) + `_max_tokens` (2000), `brand_guide_synthesis_model` (`claude-sonnet-4-6`) + `_max_tokens` (3500).
- **Tests:** +50 across the three new files + 4 wiring tests in `test_brand_guide.py` (synthesized+done folded in, regulated→awaiting_signoff reaches the row, None omits the field, no-site still synthesizes). All 193 brand-guide + content-compliance tests green locally.

**Worker verification (PENDING — post-deploy; the synthesis LLM calls go through Anthropic (proxy-reachable) but the FULL path needs a captured `brand_guides` row, and capture is egress-blocked from ScrapeOwl/DataForSEO in-sandbox):** with `brand_guide_enabled` TRUE, enqueue one job on the deployed worker for a client with a website —
`insert into async_jobs (job_type, entity_id, payload) values ('brand_guide_generate', '<client_id>', jsonb_build_object('client_id','<client_id>'));`
— then read the newest `brand_guides` row: `status='done'` (or `awaiting_signoff` for a `content_compliance_mode != 'off'` client) and **`synthesized` populated** — `synthesized->'color'->'swatches'` named/roled against the census hexes, `synthesized->'color'->'pairings'` with WCAG levels, `synthesized->>'positioning_statement'` + `voice_examples` + `boilerplate` present, `synthesized->'coherence'->'flags'` reflecting any sprawl, and `synthesized->'provenance'->>'excised_sentences'` ≥ 0 (≥1 for a regulated client whose corpus carried a claim).

**STOPPED before Phase 3** (the PDF render / render profiles) and Phase 4 (frontend + the enforcement-suggestion surface + the sign-off approval UI) per the task — each needs its own owner go. The `brand_guide_render` job type is in the CHECK but wired only as an honest "not implemented" settle until Phase 3.

## ⏩ Update — 2026-09-15 · **Brand Guide Generator — Phase 1.5 (aesthetic / "vibe" read) — BUILT + MERGED (PR [#1144](https://github.com/kssabraw/ar-tools/pull/1144))**
...
**Built (best-effort / degrade-never-fail, PRD §5.4):**
- **`services/brand_guide_vibe.py`** — ONE bounded **Claude Sonnet vision** call (`brand_guide_vibe_model`, one tier above the QA visual-check's Haiku — §12 Q4) over the **HOMEPAGE screenshot the Phase-1 capture already took** — **no DataForSEO re-pay**, and the +2 discovered pages are **not** sent (§4.3 / §12 Q4). It reuses that screenshot two ways: the generate hot path passes the **in-memory** bytes it just captured (no redundant round-trip); the standalone / re-run entrypoint reads them **back from the private `brand-guides` bucket** via `captured.pages[].screenshot_path`. Forced-tool output into a **fixed schema**, `temperature=0` for repeatability. Reuses the exact screenshot+vision pattern the QA agent runs (`qa_visual`) + its `_fit_image` sizing + the `anthropic_failover` pool client. Produces `vibe_read` = (a) **aesthetic descriptors** each with a screenshot-evidence phrase (the grounded version of "Clinical Midnight / Bio-Futuristic Sharpness" — read off the real render, never invented); (b) **mood axes** as 0–100 scales (minimal↔maximal, warm↔cool, playful↔serious, understated↔bold, budget↔premium, organic↔geometric, classic↔futuristic); (c) **character reads** (color_mood / type_personality / shape_language / spatial_density / imagery_style); plus a `methodology_note` carrying the honesty about limits (an interpretation, static shots miss motion, an LLM read can be wrong) + the model used.
- **Pure helpers unit-tested** (`tests/test_brand_guide_vibe.py`, 25): `sanitize_vibe_read` (**drops any descriptor lacking an evidence phrase** — the ungrounded-adjective guard; dedupes case-insensitively; caps count + lengths), `clamp_axis` (0–100, non-numeric → dropped not defaulted; unknown/garbage axes dropped), `homepage_screenshot_path` (homepage only — a page-1 shot never fills in). The vision call itself is mocked; the orchestrator's disabled/module-off/no-screenshot/download-miss/happy-path gates run against monkeypatched IO.
- **Wired into the `brand_guide_generate` pipeline** (`services/brand_guide.py`) as a best-effort step **AFTER the census is stored** (the screenshots are already in the bucket): `run_vibe_read_for_capture(captured)` → folded into the final `done` row write only when it returns a read; a disabled/degraded/failed read **omits `vibe_read`** (never a null write) and records an honest `captured.vibe_note`, and the guide still finalizes `done` (§5.4). A capture that degraded to CSS-only (no homepage screenshot) **skips the vibe read with a note — never fabricates a vibe**.
- **Config:** `brand_guide_vibe_enabled` (default True — the skip guard: turn the vibe read off while capture keeps running) + `brand_guide_vibe_model` (`claude-sonnet-4-6`) + `brand_guide_vibe_max_tokens` (1500). The read requires BOTH `brand_guide_enabled` (the module-wide "nothing spends until on" gate) AND `brand_guide_vibe_enabled`.
- **No migration** — the `vibe_read jsonb` column already exists (migration `20260916170000`, applied live in Phase 1). No new job type. Tests: `test_brand_guide_vibe.py` +25, `test_brand_guide.py` +2 (wiring: a returned read is folded into the row; a skip omits the field but records the note). All 39 brand-guide tests green locally; ruff clean.

**Worker verification (PENDING — post-merge-deploy; the sandbox is egress-blocked from ScrapeOwl/DataForSEO AND there is no captured `brand_guides` row on file to vibe-read against, so this can't run in-session):** with `brand_guide_enabled` TRUE, enqueue one job on the deployed worker for a client with a website —
`insert into async_jobs (job_type, entity_id, payload) values ('brand_guide_generate', '<client_id>', jsonb_build_object('client_id','<client_id>'));`
— then read the newest `brand_guides` row for that client: `status='done'`, `visual_census` populated (Phase 1), **and `vibe_read` populated** — `vibe_read->'aesthetic_descriptors'` non-empty with evidence phrases, `vibe_read->'mood_axes'` a subset of the seven axes each 0–100, `vibe_read->'character'` present, `vibe_read->>'model'` = the Sonnet id. A homepage that captured a screenshot but the read failed shows `captured->>'vibe_note'` explaining why + no `vibe_read` (guide still `done`). (The Phase-1 capture mechanics + the QA screenshot+vision pattern are both already proven live.)

**STOPPED before Phase 2** (grounded synthesis / the Proposed layer + the coherence check that cross-checks the vibe read vs the census + the §5.3 regulated guardrail) and Phase 3 (render) per the task — each needs its own owner go.

## ⏩ Update — 2026-09-15 · **Brand Guide Generator — Phase 1 (capture, no browser) — BUILT + MERGED (PR [#1141](https://github.com/kssabraw/ar-tools/pull/1141))**

Phase 1 of the Brand Guide Generator (PRD `docs/modules/brand-guide-generator-prd-v1_0.md` §4.1–4.2 / §6 / §10). Builds on the merged Phase 0 (#1138: the pure extraction core + the D4 palette spike, **gate PASSED live** — `css_recovered_any=true`, 4/4 sites captured). The ADR (no headless browser) stands. **Gated on `brand_guide_enabled` (default False in code; **TRUE on PLATFORM** — owner ruled leave it TRUE, see below). Nothing auto-enqueues, so nothing spends until a guide is explicitly requested; the new capture path is dark until this deploys.**

**Built (all best-effort / degrade-never-fail, PRD §5.4):**
- **Migration `20260916170000_brand_guides.sql` — APPLIED LIVE (verified):** the versioned `brand_guides` table (§6: `captured` / `visual_census` populated in Phase 1; `vibe_read` / `synthesized` / render / `edited` / `awaiting_signoff` are later phases), RLS service-role only, unique `(client_id, version desc)`; a private `brand-guides` storage bucket for page screenshots (kept out of `reports`, which holds the Phase-3 PDF); and the `async_jobs` CHECK **rebuilt from the LIVE constraint** + `brand_guide_generate` + `brand_guide_render` (both added now so Phases 3/4 need no further CHECK migration). **NO new regulated flag** — the future `awaiting_signoff` gate reads the existing `clients.content_compliance_mode` (nothing to gate in Phase 1: no synthesis yet).
- **`services/brand_guide.py`** — the capture pipeline: homepage (the visual authority) + up to `brand_guide_max_pages` (2) auto-discovered key pages via the existing ScrapeOwl (`render_js`) + DataForSEO `page_screenshot` production paths → the deterministic census → stored on the row (status `queued→capturing→done|error`). **Homepage is the SOLE palette/fonts/type source** (§4.1); the +2 pages add logo variety + a capture record only. Screenshots persisted to the `brand-guides` bucket (so the Phase-1.5 vibe read reuses them, no re-pay). `enqueue_brand_guide_generate()` + `run_brand_guide_generate_job()` (flag-gated; defensive — a bare `{client_id}` job insert creates its own row, so a worker-verification insert runs end-to-end). Registered in `job_worker` dispatch + `SINGLE_JOB_REGISTRY` (completion ping). NOT freeze-gated (observation/deliverable, §5.5). The Pillow `pixel_counts_from_png` helper **moved here** (canonical capture home) and re-exported from `brand_guide_spike` so both share one implementation.
- **Three Phase-0 spike findings fixed in the capture layer** (were log-only; grounded in the real spike report on freightoptics / sealbeachcolabs / umh / wellington): **(1)** the font extractor now drops `var(--…)` (Elementor) + `wfont_<hash>` / bare-hex (Wix) family leaks while keeping real Wix aliases (`futura-lt-w01-light`); **(2)** logo ranking down-ranks third-party asset hosts (cookieyes/onetrust/…) and non-brand hints (customer/partner logos, social-share badges) so the real mark leads — wixstatic/wp-content CDNs are deliberately NOT penalized (that's where a client's own logo lives); **(3)** `_capture_page` degrades cleanly to the screenshot/pixel path on a ScrapeOwl 401 and records the reason in the page notes. Plus a pure `discover_key_pages()` nav helper (§4.1).
- **Tests:** `tests/test_brand_guide_extract.py` +9 (font/logo fixes + discovery); `tests/test_brand_guide.py` new (14 — dom digest, logo merge, the degrade-on-401 path, homepage-only palette, version increment, flag gate, Pillow quantize). All 57 green locally; ruff clean.

**Owner decision (made 2026-09-15) — leave `brand_guide_enabled` TRUE on PLATFORM.** It stays TRUE (set originally for the Phase-0 spike); nothing auto-enqueues so there's no runaway-spend risk, and TRUE is what lets a post-deploy worker-verification job run. Flip to FALSE only if the module should be inert until Phase 4's UI lands.

**Worker verification (post-merge-deploy — the sandbox is egress-blocked from ScrapeOwl/DataForSEO, same as the spike):** with the flag TRUE, insert one job on the deployed worker —
`insert into async_jobs (job_type, entity_id, payload) values ('brand_guide_generate', '<client_id>', jsonb_build_object('client_id','<client_id>'));`
— then read the newest `brand_guides` row for that client: `status='done'`, `visual_census->>'palette_source'` = `pixel`, `captured->'pages'` has the homepage + up to 2 discovered pages with screenshot paths + honest capture notes. (The D4 spike already proved the underlying capture mechanics live.)

**STOPPED before Phase 1.5** (the Sonnet-vision aesthetic/vibe read) per the task — that + Phase 2 (synthesis) + Phase 3 (render) each need their own go. `brand_guide_render` is in the CHECK but wired only as an honest "not implemented" settle until Phase 3.

## ⏩ Update — 2026-09-15 · **Brand Guide Generator — Phase 0 BUILT + MERGED (PR [#1138](https://github.com/kssabraw/ar-tools/pull/1138), squash `01342037`); the D4 palette-method gate RAN LIVE and PASSED**
...
**Built + merged (squash `813b323`; CI green — platform-api tests + lint/typecheck + Netlify preview):**
- **Migration `20260915160000_paa_manifests.sql` — APPLIED LIVE (verified):** `paa_manifests` (one per `paa_set`, unique `set_id`; `cost_summary`/`qa_summary` roll-ups + last Sheet-export refs) + `paa_manifest_assets` (assets **first-class** — a QA verdict + a cost attach per row: `category` content/authority/media, `source` auto/seed/manual, `kind`, `url`, `status`, `confidence_tag`, `cost_task_type`, `qa_verdict`/`qa_review`, `paa_item_id`). New `async_jobs` type `paa_manifest_qa` (drift-proof CHECK widen). Both RLS-on, service-role.
- **`services/paa_manifest.py`** (pure, unit-tested `tests/test_paa_manifest.py`): the seeded **authority bundle** (the 7 seam bolts, reference §5.1 — tracked-only, confidence-tagged, mapped to a Recipe-Engine cost where one exists) + the audio/video/influencer manual rows; content-row assembly from resolved v1 linkage; **rebuild reconciliation** (`merge_rows` — refresh the auto rows, PRESERVE operator edits); the reused `recipe_engine.cost_of` rollup (honest **"not estimated"** for off-menu RD 100); the QA rollup; CSV / Sheet / JSON export rendering.
- **`services/paa_manifest_service.py`** (I/O; `tests/test_paa_manifest_service.py`): build/refresh (resolves `runs.published_url` / `gbp_posts.search_url` / syndication copies), asset CRUD, the `paa_manifest_qa` async job (reuses `qa_service.review_url` per **live content URL** — the thing authority amplifies, reference §2 — gated on `qa_enabled`, v1's deterministic per-item checks as the free fallback), and the Google-Sheet export into the client's Drive folder (`google_docs.create_google_sheet`).
- **`routers/paa.py`** (+ `models/paa.py`): build/get manifest, QA enqueue, asset add/patch/delete, CSV/JSON export, Sheet export. `job_worker` dispatch for `paa_manifest_qa`. **Frontend** (`pages/PaaSets.tsx`, `lib/types.ts`): a **Prep Sheet** section on the PAA-set detail expander — build/rebuild, cost + QA summaries, the asset table grouped by category with editable status on authority/media rows, **Run QA**, and CSV / JSON / Google-Sheet exports. Ships as a plain content surface (**no feature flag**; manifest QA rides `qa_enabled`).

**Reuse (didn't rebuild):** `paa_items` v1 linkage · `runs.published_url` · `gbp_posts.search_url` · `syndication_items` · `recipe_engine.price_catalog`/`cost_of` · `qa_service.review_url` · `google_docs.create_google_sheet` + `resolve_drive_folder`.

**Deferred — Phase 3 (needs its own owner greenlight — PRD §6):** the Service PAA Campaign object + the **automated** single-variable gate/verify state machine (v1 documents it as a *manual* workflow; reuses Maps geo-grid + response-episodes). **Not wired into any agent loader** (`sop_library` never reads the master reference). The pre-existing `test_pace_interventions.py` date-bomb passed in this run.

## ⏩ Update — 2026-09-15 · **PAA → SEO Neo v1 (the CONTENT HALF) — BUILT, MERGED (PR [#1117](https://github.com/kssabraw/ar-tools/pull/1117)), LIVE in production**
...
- **Migration `20260916120000_google_trends_phases234.sql` — APPLIED LIVE** (verified): `google_trends_runs.mode` (keyword|category|portfolio|local_seasonal) + `client_id` made **nullable** (portfolio runs are client-less) + `google_trends_keywords.relevance_score`/`audience_fit`/`source_client_name`.
- **Phase 1.1** — a "Write this post" CTA on a qualified rising-query row (`pages/GoogleTrends.tsx`), cloning KeywordResearch's `topicSeedKeyword`/`composeWriterNotes` → `POST /runs` (blog run seeded with the query + the trend angle in `writer_notes`). Frontend-only.
- **Phase 2 (informational)** — a SEEDLESS **category scan** (`run_google_trends_category_scan`): seeds derived from the client's site topics/ICP (`keyword_research_topics.research_topics`), gated by the SAME `keyword_research_relevance` + `keyword_research_audience` filters, survivors routed to Topic Research via a frontend "Send to Topic Research" button → the existing `POST /clients/{id}/topic-research` (explicit human click, so the expensive strategist run isn't auto-spent). Endpoint `POST /clients/{id}/google-trends/category-scan`.
- **Phase 3 (portfolio)** — a **weekly agency sweep** (`run_portfolio_trends_sweep`) over the union of clients' tracked keywords (no client scope, no relevance/audience gate) → a deterministic `trends_digest` notification. Routing: `trends_digest` is in NEITHER the PACE nor DIRECTOR set, so it **falls through to `slack_default_channel` (the SerMaStr strategy channel)** — no routing code needed. Wired weekly on the shared `gsc_scheduler` (`enqueue_due_portfolio_trends_sweep`, self-gated on the flag, marker `trends_sweep_weekly`, weekday `google_trends_portfolio_weekday`=Mon) + on-demand admin `POST /google-trends/portfolio-sweep`; reads `GET /google-trends/portfolio[/runs/{id}]`.
- **Phase 4 (local seasonal, metro-geo only)** — pure `parse_interest_over_time` (the `google_trends_graph` slice) + `seasonality_profile_from_series` (the exact `{"index":{1..12}, "peak_months"}` shape `trend_watch.demand_outlook` consumes) + `run_local_seasonal_scan` (metro-gated: requires a `location_code`; one explore per keyword over a 24-month window; result rides the job row, **no new table**). Endpoint `POST /clients/{id}/google-trends/seasonal`. Config `google_trends_seasonal_months`.
- Pure helpers unit-tested (`tests/test_google_trends.py`, now **25** — added Phase 2/3/4 cases). New config: `google_trends_category_seed_cap`, `google_trends_portfolio_weekday`/`_max_clients`/`_seeds_per_client`/`_digest_size`, `google_trends_seasonal_months`.
