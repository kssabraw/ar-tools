...
- **Wired onto the THREE scorers only** (`/score-page` local+service, `/score-ecommerce-page`, `/score-blog-page`) — matching P1's coverage; the reopt endpoints (final-response + coaching) call the wrapper WITHOUT the arc kwargs, so they're byte-identical (report-only, no coaching — like P0 centering).
