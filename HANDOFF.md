...
## ⏩ Update — 2026-09-02 · **DORA guide sync — every user-facing module change is reported to DORA, which rewrites the module's in-app guide when it stops being true (built; needs two secrets to activate)** (latest)
...
- **Migration `20260902170000_guide_sync.sql`** (`guide_sync_runs` + the `guide_sync` job type in the `async_jobs` CHECK, rebuilt from the live list) — **apply it live before/with the deploy**.
...
- **BULK lanes = the throughput knob.** `bulk_lane_workers` (default **1**) spawns N `lane="bulk"` workers that claim only `priority <= -1`. 1 keeps today's two-in-flight (bulk lane + MAIN); 4 runs four pages at once. The nlp container serves every lane — watch its memory on Railway as you raise it, and size the key pool to match.
