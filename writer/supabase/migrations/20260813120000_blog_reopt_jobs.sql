-- Blog article reoptimization support.
--
-- 1. runs.reoptimize_source_html — a blog article pasted into the WYSIWYG editor
--    (the paste-content reoptimize input), alongside the existing
--    reoptimize_source_url (a live URL to scrape). Either seeds a blog run's
--    Stage B' scoring so the writer's first pass fixes the source's deficiencies.
alter table runs add column if not exists reoptimize_source_html text;

-- 2. async_jobs.job_type — add the blog score / reoptimize job types. The live
--    CHECK is wider than any single repo migration, so this reproduces the full
--    current set and appends the two new values (blog_score, blog_reoptimize).
