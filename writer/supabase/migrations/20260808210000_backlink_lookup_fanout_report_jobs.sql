-- Two more synchronous flows move to background async_jobs so the user can
-- navigate away while they run and get a completion notification in the activity
-- sidebar: the any-domain Backlink lookup (`backlink_lookup`, a DataForSEO pull
-- on a cache miss) and the Fanout keyword-research PDF report (`fanout_report`,
-- WeasyPrint render). Widen the job_type CHECK preserving the FULL LIVE set.
