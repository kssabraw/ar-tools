-- Reconcile the async_jobs job_type CHECK: include BOTH 'ga4_ingest' (re-added by
-- migration 20260815210000_async_jobs_readd_ga4_ingest) AND 'gbp_reviews' (added by
-- 20260815210000_gbp_reviews_job). Those two share a timestamp and independently
-- recreate the CHECK, so whichever replays last drops the other's type. This
-- later-timestamped migration recreates the CHECK once with the FULL allow-list,
-- so a fresh migration replay converges correctly and live matches the files.
