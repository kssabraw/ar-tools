-- Autonomous SEO agent — Phase 3: register the `autonomy_run` async job type.
-- Applied live from the DB's actual constraint (the live job_type set is wider
-- than any single repo migration file — see CLAUDE.md), preserving every
-- existing type and appending 'autonomy_run'. Inert until autonomy_enabled.
