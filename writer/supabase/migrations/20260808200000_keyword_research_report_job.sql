-- Keyword Research PDF report moves from a synchronous request (Claude
-- exec-summary + WeasyPrint render + two uploads) to a background async_jobs
-- job (`keyword_research_report`), so the user can navigate away while it
-- builds and gets a completion notification in the activity sidebar. Add the
-- new job_type (preserving the FULL LIVE set — wider than any single repo
-- migration file; read from the live constraint) and give the reports table an
-- `error` column so a failed background render can record why.
alter table keyword_research_reports add column if not exists error text;
...
alter table async_jobs add constraint async_jobs_job_type_check check (
  job_type in (
    ...
    -- Present in the live constraint (added after the repo file above); kept so
    -- ordered application ends with the full live set.
