-- Run-free "Score an existing page" jobs for the Blog + Service writers:
-- register the `score_external` async job type. Applied live from the DB's actual
-- constraint (the live job_type set is wider than any single repo migration file
-- — see CLAUDE.md), preserving every existing type and appending 'score_external'.
