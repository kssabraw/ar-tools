-- GBP reviews async job. Fetches a location's reviews from the Google My Business
-- v4 reviews API (the same v4 surface GBP Posts uses) for the reporting dashboard's
-- "reviews this period" panel. First-party alternative to the Outscraper feed in
-- services/gbp_reviews.py (which powers the client PDF report).
-- Add 'gbp_reviews' to the async_jobs job_type CHECK, recreating it while
-- preserving the full current live allow-list.
