-- LeadOff GBP Placement Advisor — add the `leadoff_placement` async job type.
-- The job fills the Census block-group demand cache (census_block_demand) for a
-- market's counties on the first advisor run; placement zones are computed on
-- read. Recreate the async_jobs job_type CHECK preserving the full current live
-- allow-list (read from the live DB 2026-08-25) and appending the new type.
