-- LeadOff live-GBP map refresh async job. Re-pulls only the market map's
-- competitor GBP pins (one Maps SERP, ~$0.004) — decoupled from a full scout —
-- so a fully-cached scout (or a never-scouted market) can (re)generate its map
-- without the full enrichment pull or a 90-day cache wait.
-- Add 'leadoff_map_refresh' to the async_jobs job_type CHECK, recreating it
-- while preserving the full current live allow-list.
...
alter table public.async_jobs add constraint async_jobs_job_type_check check (
  job_type = any (array[
    ...
    'ga4_ingest', 'gbp_reviews', 'leadoff_map_refresh'
  ]::text[])
);
