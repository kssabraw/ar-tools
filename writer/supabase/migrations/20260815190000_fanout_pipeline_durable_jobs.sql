-- Durable Fanout pipeline (issue #686 Phase 3).
--
-- Phase 1 moved `expand` onto the durable async_jobs queue (job_type
-- `fanout_expand`). Phase 3 does the same for the remaining pipeline
-- entrypoints so EVERY long Fanout run survives a platform-api restart/deploy:
-- `plan-articles`, `regate`, recursive `fanout`, and site `architecture`.
--
-- Additive: drop + re-add async_jobs_job_type_check with the full current live
-- list plus the four new types (the established pattern — the live constraint is
-- the source of truth and is wider than any single repo migration file).
alter table public.async_jobs drop constraint if exists async_jobs_job_type_check;

alter table public.async_jobs
  add constraint async_jobs_job_type_check check (job_type = any (array[
    ...
    'brand_scan','brand_report','notification_dispatch','reopt_plan','client_report',
    'maps_analyze','asana_monthly','competitor_gbp','review_intel','backlink_intel',
    ...
    'backlink_lookup','fanout_report','blog_score','blog_reoptimize','fanout_expand',
    'gbp_onboard','gbp_search_keywords',
    -- issue #686 Phase 3: the remaining Fanout pipeline entrypoints go durable.
