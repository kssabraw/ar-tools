-- Add the 'gbp_onboard' async job type (bulk-onboard clients' Google Business
-- Profiles into gbp_locations + kick off metric backfills). Recreates the
-- async_jobs.job_type CHECK with the full current allow-list plus the new value.
-- (The live constraint has historically been the source of truth and wider than
--  any single repo migration; this preserves every existing value verbatim.)
