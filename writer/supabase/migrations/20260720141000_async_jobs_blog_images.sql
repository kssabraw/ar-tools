-- Add the blog_github_publish job type (generate hero + body images with
-- gpt-image-1, then commit the markdown + image bytes to the client's repo in
-- one atomic commit). Recreates the CHECK from the exact live definition + the
-- new value; strictly additive.
