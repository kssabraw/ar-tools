-- Social: move per-client PostPeer profile (Social group) provisioning off the
-- synchronous client-create request path onto the async worker, matching every
-- other provisioning step (all of which enqueue an async_jobs row rather than
-- making a blocking external call inline). Adds the job_type.
--
