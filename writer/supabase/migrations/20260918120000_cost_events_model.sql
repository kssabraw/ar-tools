-- cost_events, part 4: add a `model` dimension so the admin Cost & Usage Report
-- can break spend + tokens down by the model that did the generation
-- (Claude Sonnet / Haiku / OpenAI GPT-5.6 "Luna" / …), not just by type / client
-- / member.
--
-- The model lives in a different place per source, so the view normalizes it:
--   * local_seo_pages / ecommerce_pages → token_usage->>'model' (the effective
--     prose model — Sonnet or gpt-5.6-luna), falling back to the cost_breakdown
--     claude_model, then Sonnet (the always-configured default).
--   * strategy_reviews → its `model` column (Sonnet default).
--   * qa_reviews → Haiku (the QA agent's fixed model; its token_usage stores no
--     model id).
--   * runs (blog / service) → 'mixed': the pipeline spans several models
--     (brief/SIE/research/scoring on Claude, prose on Claude OR OpenAI) and only a
--     scalar total_cost_usd is stored, so a single model can't be attributed
--     honestly — it's reported as one "mixed pipeline" bucket.
--   * keyword_research / topic / domain_intel / leadoff / autonomy → NULL: these
--     are paid non-LLM API calls (DataForSEO, etc.), not model token spend.
--
-- Column order is unchanged and `model` is appended last (CREATE OR REPLACE VIEW
-- only allows adding columns at the end). Estimated-cost and token logic is
-- otherwise identical to migration 20260904160100.
...
union all
-- QA reviews (Haiku; token_usage carries no model id) -------------------------
select
