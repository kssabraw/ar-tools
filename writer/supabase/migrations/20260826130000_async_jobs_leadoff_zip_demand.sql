-- LeadOff GBP Placement Advisor — Phase 0b/3 ZIP-demand job.
-- The `leadoff_zip_demand` job runs the Phase 0b feasibility probe (~$0.05
-- DataForSEO per-ZIP volume check) and, if it passes, becomes the Phase 3 paid
-- ZIP-demand fetch. Recreate the async_jobs job_type CHECK preserving the full
-- current live allow-list (read from the live DB 2026-08-26) + the new type.
