-- Migration: 20260713000100_async_jobs_ecommerce.sql
-- Purpose: Allow the Ecommerce Writer's async job types:
--            * ecommerce_generate        — write a new product/collection page
--            * ecommerce_reoptimize_url  — score→reoptimize a live page by URL
--            * ecommerce_action          — interactive actions (score, discover,
--                                          related) run backgrounded + polled
--          Strictly additive. The full existing set is preserved verbatim from
--          the LIVE constraint (which had drifted wider than any repo migration
--          file — see CLAUDE.md note on the async_jobs CHECK), so no
...
alter table async_jobs
  ...
  check (job_type = any (array[
    ...
    'leadoff_city_finder', 'leadoff_income_backfill',
