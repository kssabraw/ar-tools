-- Service-page score / reoptimize move from in-request SSE to background
-- async_jobs (`service_page_score` / `service_page_reoptimize`) so a deploy
-- can't kill them — the job is requeued and finishes server-side, and the UI
-- reconnects instead of telling the user to run it again. Widen the job_type
-- CHECK preserving the FULL LIVE set (which is wider than any single repo
-- migration file — always read it from the live constraint, not from repo
-- history).
...
alter table async_jobs add constraint async_jobs_job_type_check check (
  job_type in (
    ...
    -- From 20260713000000_deliverables_sheet.sql, whose CHECK rewrite predates
    -- (and so omits) the two service_page types below — this later migration
    -- must stay the superset so ordered application ends with every type live.
