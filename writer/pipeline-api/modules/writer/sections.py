...
def _build_section_user_prompt(
    ...
    # Article-level frame so the section knows the surrounding context.
    # Without these the LLM writes each section in relative isolation,
    # which causes (a) tonal drift section-to-section and (b)
    ...
    if preceding_section_summaries:
        ...
        for summary in preceding_section_summaries:
    ...
    # Client's structural style (heading depth, length variation, recurring
    # blocks). Soft guidance applied over this section's outline — never overrides
    # the required structural rules or term/citation directives above.
    if reference_structure and reference_structure.strip():
        parts.append("\n" + reference_structure.strip())

    # Per-run editorial guidance typed by the user at run creation. Every
    # section prompt sees the same notes; the directive below keeps a
    # specific-inclusion note ("mention <brand> as one of the top 10") from
    # being force-fed into every section.
    if user_notes and user_notes.strip():
        parts.append(
            "\nUSER_NOTES (editorial guidance from the account team for this "
            "article - honor it faithfully. If a note asks for a specific "
            "mention or inclusion, work it in where it fits most naturally "
            "given this section's place in the outline; do NOT repeat the "
            "same insertion in every section. Notes never override "
            "FORBIDDEN_TERMS, citation rules, or the required structure):"
        )
