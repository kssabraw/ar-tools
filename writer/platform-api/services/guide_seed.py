"""Default Guides content — seeded into the `guides` table at startup.

This is the initial/default copy of the in-app help portal. Once seeded, the DB
table is the source of truth and the team edits guides in-app; this list only
provides defaults for fresh environments (and any newly-added slugs). Seeding is
idempotent on slug (guide_store.seed_defaults), so editing or deleting a guide in
the app is never clobbered by a redeploy.

Bodies use the Markdown subset the frontend's Markdown component renders
(#/##/###, **bold**, - bullets, pipe tables, --- rules).
"""

from __future__ import annotations

DEFAULT_GUIDES: list[dict] = [
    {
...
## What it gives you
- Per-engine visibility, an SVG trend line, and a keyword x engine **mention matrix**.
...
On-demand opportunity analysis from a live Search Console query x page pull (~90 days). Surfaces three opportunity types.

## What it finds
- **Cannibalization** — a query split across multiple URLs, none ranking well (Google can't pick a page).
- **Quick wins** — query x page at positions 6-10.
- **Hidden wins** — query x page at positions 11-30 with demand.
