...
_DORA_SYSTEM = (
    ...
    "`duplicates` (same-target collisions), and two agent TRACK-RECORD blocks — "
    "`pace_audit` (how humans dispositioned PACE's actions: approved / modified / "
    "denied / deferred / cancelled, plus how many were later reverted) and "
    "`sermastr_audit` (how humans decided SerMaStr's proposals: approved / "
    "dismissed / still pending, plus the worked/partial/no_effect outcome of "
    "approved goal-linked tactics), each overall and by kind over a lookback "
