...
async def load_source(client_id: str, req) -> tuple[Optional[str], str, dict]:
    """Resolve the requested Source to (title, plain_text, source_ref). Content
    reads are scoped to the client (a client can only repurpose its own content).
    Raises HTTPException on a bad/unreachable source."""
    st = (req.source_type or "topic").lower()
    ...
    if st == "topic":
        ...
        return None, text[:cap], {"type": "topic"}
    ...
    if st == "url":
        ...
        try:
            title, markdown = await extract_source_content(url)
        ...
        return title, (markdown or "")[:cap], {"type": "url", "url": url}

    if st == "blog_run":
        run_id = (req.source_id or "").strip()
        if not run_id:
        ...
        text = sections_to_text(_load_article(sb, run_id), cap)
        return run_rows[0].get("keyword"), text, {"type": "blog_run", "run_id": run_id}

    # local_seo_page
    page_id = (req.source_id or "").strip()
    if not page_id:
        raise HTTPException(status_code=422, detail="social_source_id_required")
    ...
    if not rows:
        raise HTTPException(status_code=404, detail="social_source_not_found")
    text = strip_html(rows[0].get("content_html") or "")[:cap]
    return rows[0].get("page_title"), text, {"type": "local_seo_page", "page_id": page_id}
...
async def generate_copy(client_id: str, req, user_id: Optional[str] = None) -> dict:
    ...
    from services import gbp_posts_service, report_llm, voice_card_service

    spec = _platform_spec(platform)
    source_title, source_text, source_ref = await load_source(client_id, req)
    notes: list[str] = []
    ...
    while hits and passes < int(settings.social_copy_max_correction_passes):
        ...
        try:
            rewritten = clamp_copy(await _one_call(system, fix_user), char_limit)
