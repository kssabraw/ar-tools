...
def _prov_gbp_metrics(supabase, client_id: str, today: date, now: datetime) -> Optional[dict]:
    """GBP Insights PERFORMANCE — profile views + customer actions (calls,
    website clicks, direction requests, messages), trailing 30 days vs the prior
    30, summed across the client's verified locations. The demand-side read on
    the local pack: distinct from `gbp_audit` (listing completeness) and
    `maps_geogrid` (rank coverage) — this is whether the listing is actually
    being seen and acted on. Reuses the dashboard's growth math so the digest and
    the GBP Insights dashboard agree. Dormant until `gbp_metrics_enabled`."""
    ...
    window = 30
    # Anchor at the last date with data — GBP performance lands ~3–5 days late,
    # so anchoring at `today` would read the not-yet-arrived tail as a decline.
    latest = (
        supabase.table("gbp_metric_daily").select("date")
        .in_("location_row_id", loc_ids).order("date", desc=True).limit(1).execute()
    ).data
    if not latest:
        return None
    try:
    ...
    # PostgREST's 1000-row cap, and a truncated read would skew the growth math.
    rows: list[dict] = []
    page, offset = 1000, 0
    while True:
        batch = (
            ...
            .order("date").range(offset, offset + page - 1).execute()
        ).data or []
        rows.extend(batch)
        if len(batch) < page:
            break
    ...
    return {
        ...
        "as_of": end.isoformat(),
        "metrics": [
            {"metric": c["label"], "current": c.get("current"),
             "previous": c.get("previous"), "pct_change": c.get("pct")}
            for c in cards
        ],
        "note": (
            "GBP performance over the trailing 30 days vs the prior 30, across the "
            "client's verified locations. 'Profile views' is how many saw the "
            "listing on Search + Maps; calls / website clicks / direction requests "
            "/ messages are the actions taken from it. TRAP: this is DEMAND (was "
            "the listing seen + acted on), not rank — falling views with steady "
            "geo-grid rank points at seasonality or a listing/prominence issue, "
            "not a ranking drop. pct_change is None when the prior window had no "
            "baseline. Data lands ~3–5 days late — `as_of` is the last day with "
            "data, not today."
        ),
    }


def _prov_ga4(supabase, client_id: str, today: date, now: datetime) -> Optional[dict]:
    """Google Analytics (GA4) website traffic — sessions, pageviews, conversions,
    and top channels, trailing 30 days vs the prior 30. The site's own demand +
    conversion read, complementary to rank/impressions. Reads `ga4_daily`; None
    without a verified property. Deliberately omits summed `total_users` (GA4
    de-dupes users per day, so a sum overstates unique visitors)."""
    prop = (
    ...
    rows = (
        ...
        .eq("property_id", prop[0]["id"]).gte("date", cutoff).order("date").execute()
    ).data or []
    ...
    out: dict = {
        ...
        "note": (
            ...
            "omitted (a daily sum overstates them). pct_change is None when the "
            "prior window isn't covered. `as_of` is the last day with data. Read "
            "organic-channel sessions + conversions together with rank/impressions "
            "— rising positions with flat organic sessions is a CTR/SERP-feature "
