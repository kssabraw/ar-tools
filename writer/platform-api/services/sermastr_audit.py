...
def _blank_stats() -> dict:
    return {"approved": 0, "dismissed": 0, "pending": 0, "superseded": 0,
            "expired": 0, "worked": 0, "partial": 0, "no_effect": 0, "total": 0}
...
def _tally_stats(bucket: dict, row: dict) -> None:
    ...
    if dec == "approved":
        ...
        # Aged out un-actioned past the expiry window — a system event, never a
        # human verdict, so (like superseded) it counts in neither rate.
        bucket["expired"] += 1
    else:
        bucket["pending"] += 1
...
def record_decision(*, review_id: str, idx: int, proposal: dict,
    ...
    if not settings.sermastr_audit_enabled:
        return
    if decision not in ("approved", "dismissed", "superseded", "expired"):
        return
...
def record_expired(*, review_id: str, idx: int, proposal: dict,
                   client_id: Optional[str] = None, client_name: Optional[str] = None,
                   trigger: Optional[str] = None) -> None:
    """A proposal aged out un-actioned past the expiry window
    (services/strategist_expiry.py). Recorded as its OWN decision value, by the
    system — excluded from the approve/dismiss learning rates. Best-effort."""
    record_decision(
        review_id=review_id, idx=idx, proposal=proposal, client_id=client_id,
        client_name=client_name, trigger=trigger, decision="expired",
        actor_profile_id=None, actor_role=None, actor_source="system",
    )
