...
_RANKING_STRATEGY_NUDGE = (
    ...
    "WHAT COUNTS AS THE KEYWORD BEING SUPPLIED — read this carefully, because "
    "getting it wrong is the known failure of this instruction. It counts ONLY "
    "if a HUMAN named a service or search term for THIS location, in this "
    "message or earlier in this conversation. It does NOT count that the client "
    "already has tracked keywords, geo-grid keywords, ranking pages or GSC "
    "queries: those were chosen for the places they already operate, and a new "
    "location is a new commercial decision that belongs to the teammate, not to "
    "you. Two specific traps: the client's own NAME appearing next to the verb "
    "('rank <client> in <city>') is the SUBJECT of the sentence, never the "
    "keyword; and a city named in the question is the WHERE, never the WHAT. So "
    "if the only thing the teammate named is who and where, you are missing the "
    "keyword — ask for it. When you ask, make it easy: name two or three "
    "plausible candidates drawn from what the client actually sells (their "
    "existing keyword themes, their service pages) as options they can pick or "
    "correct, and say why the choice changes the plan.\n\n"
    "IF THE REQUEST (or the conversation so far) ALREADY SUPPLIES BOTH — by the "
    "strict test above — do NOT "
