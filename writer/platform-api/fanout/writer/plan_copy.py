...
def build_primary_backfill(cluster_rows: list[dict], clusters: list[dict],
                           cluster_map: dict[str, str], pk_map: dict[str, str]) -> list[dict]:
...
def copy_plan_to_client(*, source_session_id: str, target_client_id: str, user_id: str) -> dict:
    ...
    # 5) Back-fill clusters.primary_keyword_id (keywords now exist).
    inserted_pk_ids = {r["id"] for r in keyword_rows}
    backfill = [
        row for row in build_primary_backfill(cluster_rows, src_clusters, cluster_map, pk_map)
