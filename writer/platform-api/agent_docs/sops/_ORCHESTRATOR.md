...
- **Every published article/page carries a separate `<title>` and H1** — two distinctly worded strings, never the same text: a meta title (`<title>` tag / WP post title) and the on-page H1. The meta title has **no character limit**, leads with the primary keyword, includes **2–4 entities** central to the topic (specific products, people, places, or concepts), is **written for the target audience** in that audience's vocabulary, and carries no brand suffix (the site's SEO plugin appends that). **Both the meta title and the H1 include the current year whenever appropriate** for the topic (recency-sensitive, fast-moving, or version-specific subjects; omitted only for genuinely evergreen topics). Implemented in the writers (suite Blog Writer: brief `title` (meta) vs `h1`; Topic Fanout writer: brief `seo_title` (meta) vs `title`/H1); WordPress publishes send the meta title as the post title with the H1 as the body's first element. A legacy article with no distinct meta title publishes with its H1 as the fallback — acceptable, but new content must generate both.
